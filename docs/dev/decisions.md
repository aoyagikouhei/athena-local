...
- `is_create_table_as` に渡す語列の非対称（`classification` 側は先頭の `(` を除いた後、`results.rs` 側は括弧付き）は変えない。代わりに `is_create_table_as` が語ごとに先頭の `(` を外し、`AS(` を `AS` と残りに分けてから `AS` の次の語を見るので、どちらの語列でも同じ答えになる。CTAS とみなす `AS` の次の語は `SELECT`／`WITH`／`VALUES`／`TABLE` で、本物は括弧の有無・`AS(`・`VALUES`・`TABLE` のどれも、実行が失敗しても `CREATE_TABLE_AS_SELECT` と `tables/<id>` にする（2026-09-25 実測。[measurements/statements.md](measurements/statements.md)）。（#17、2026-09-18。#195、2026-09-25 で「吸収している」を訂正。#199、2026-09-25 で割れていた判定を本物に揃えた）
