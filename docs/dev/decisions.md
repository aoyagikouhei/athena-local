...
- 本物が `StartQueryExecution` の時点で弾く、引用符付きの名前を取る DDL 系の文の判定は、新モジュール `operation/quoted_names.rs`（純関数、athena-local 側）に置き、`classification`（`StatementType`／`SubstatementType` の判定）とは別の関心として扱う。SQL は書き換えず読むだけで、`athena-sql` crate には足さない（crate は SQL が正しいかどうかを判定しない。上の「SQL の本文は書き換えない」・「SQL の内部 crate」の決定と両立させる）。キーワードの並びは `target_table::table_name_start` を `pub(super)` にして共有し、同じ判定を作り直さない。（#204、2026-09-25）
...
- S3 Tables の Context で `AwsDataCatalog.<名前空間>.<表>`（小文字ちょうどでない `awsdatacatalog`）は、本物は 1 部目を無視して名前空間に作る。本物どおりに作るには SQL の 1 部目を落とす書き換えが要り、書き換えない約束に反するので、名前空間が無いときだけ Trino に送らずに本物と同じ FAILED（`Failure::cannot_find_table`、結果ファイルも `.metadata` も置かない）にし、あるときは No location のまま。3 案（No location のまま・無いときだけ FAILED・Trino にそのまま渡す）のうち実測に一番近い。FAILED は開始時に `Submission::immediate_failure` として持たせ、`spawn_query` が `mark_running` の後で `finish` する（取り消しとの競合は既存のガードに任せる）。1 部目が Trino にある他のカタログのときは未実測なので名前空間を見ない。2 部の `<db>.<t>` は #231、別カタログの名前の CTAS は #232 に分けた。（#227、2026-09-26、ユーザーの判断）
