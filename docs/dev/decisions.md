...
- `quoted_names::rejection` は純関数のまま変えず、存在の確認は別モジュール `entity_check.rs` に置いて 3 択の列挙型（`Reject`／`Run`／`Continue`）を返す構成にする。`Reject` は開始時にそのまま弾く、`Run` はビューなので `quoted_names` を呼ばずに実行へ進む、`Continue`（対象外の文・4 部以上・`TRINO_CATALOG_MAP` の別名・実在するテーブル・確かめられなかった場合）は今までどおり `quoted_names` を呼ぶ。4 部以上は `parse_target_table` が None を返し、別名は `config.catalog_map` に一致した時点で問い合わせずに `Continue` を返すので、判定の順序（4 部以上 → 別名 → カタログの有無 → テーブルの有無 → ビュー → 引用符付きの名前）は `entity_check` の中だけで保たれ、`quoted_names` 側の判定は変えずに済む。（#207、2026-09-25）
