...
- `is_create_table_as` に渡す語列の非対称（`classification` 側は先頭の `(` を除いた後、`results.rs` 側は括弧付き）は変えない。`SELECT`／`WITH`／`(` の 3 通りの OR がこの差を吸収するのは `AS (SELECT`／`AS ( SELECT`／`AS (WITH` の形だけで、`AS (VALUES 1)`・`AS (TABLE x)` ではファイル名と SubstatementType が割れる（本物の値を測って揃える。#199）。#195 では挙動を変えず固定テストに今の値を焼き込んだ。（#17、2026-09-18。#195、2026-09-25 で「吸収している」を訂正）
