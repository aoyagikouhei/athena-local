...
- `AthenaErrorCode` 付きのエラーの本文は `{"__type","AthenaErrorCode","ErrorCode","Message"}`（`ErrorCode` は `AthenaErrorCode` と同じ値、キーは大文字始まりの `Message`）。`x-amzn-errortype` ヘッダは本物に無かったが、消す理由も無い（SDK は `__type` を先に見る）ので残す。（#3、2026-09-17）
...
- 挙動を変えない分割では、1 関数の内部を小関数に割らない（起票もしない）。理由: 分岐の形を書き換えると「1 行も変えない」保証が弱くなる。（#19、2026-09-18、#30、2026-09-20、#44、2026-09-21）
