...
- カタログ不在の `DATACATALOG_NOT_FOUND` は、名前に 3 部書いてカタログまで指定した形だけで返す。既定・文脈のカタログが Trino に無いとき（名前が 1〜2 部で省略した既定のカタログが無い場合）は未実測なので、存在の確認では判定せず今までどおり実行する。（#207、2026-09-25）
