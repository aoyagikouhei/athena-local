...
- 実在しない QueryExecutionContext の Catalog は、本物が既定のカタログで成功させたメタデータの文（`DESCRIBE_TABLE`・`SHOW_COLUMNS`・`SHOW_TABLES`・`SHOW_DATABASES`・`SHOW_CREATE_TABLE`・`DROP_TABLE`）だけ差し替える。表を読む SELECT・EXPLAIN・CTAS は本物も失敗させたので差し替えず、Trino の `CATALOG_NOT_FOUND` を ErrorType 1006 にする（文言の `not found` ↔ `does not exist` と、本物の CTAS の 1300 は変換しない）。差し替え先は `TRINO_CATALOG_MAP` の `AwsDataCatalog` の別名（キーは大文字小文字を区別しない）、無ければ `TRINO_CATALOG`（ユーザーの判断。本物の既定が AwsDataCatalog なので、その名前に忠実にする）。有無は `system.metadata.catalogs` に小文字で問い合わせて決め、Context にカタログがあり別名でもないメタデータの文のときだけ問い合わせる。問い合わせが失敗したり形が違ったりしたら今までどおり。判定は `context_catalog::resolve` 1 か所に置き、開始時（`entity_check` の前）と実行時（`run`）の 2 回呼ぶ（#207 の「2 回飛ぶのは受け入れる」と同じ理由で Store に持ち越さない）。`resolve` は差し替えたとき Trino 側の名前を返す: 別名のキーを返すと `entity_check` の「別名なら問い合わせない」に当たって、実在しない表を開始時に弾けない（計画レビュー）。Store・Fingerprint・GetQueryExecution の Catalog は受け取った値のまま。（#214、2026-09-25）
