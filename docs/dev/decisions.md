...
- 弾くのは 2 ラウンドの実測で確かめた形だけにし、実測していない形（3 部の `ALTER TABLE` 名で 2 番目だけ引用符付き、4 部以上の名前、`ALTER TABLE IF EXISTS ...` に引用符付きの名前、`SHOW TABLES IN`／`CREATE TABLE` の 2 部以上の名前、`DESCRIBE`／`DESC` の引用符付きの部分に非 ASCII を含む形）は今までどおり実行する。文言を実測できない形を推測で埋めない、というユーザーの判断による。（#204、2026-09-25、ユーザーの判断 D2）
