...
- [CLAUDE.md](../../CLAUDE.md) の「開発上の約束」とアーキテクチャの節にすでに書いてある方針（本物の Athena に合わせる、推測で埋めない、SQL の本文を書き換えるのは実測した場合だけ、`mod.rs` は宣言と再エクスポートだけ、など）はここに重ねない。
...
- 新しい issue に着手したら、ブランチを切った直後に過去の issue のノート（`.claude/issue-notes`）を全部消す。Skill が新しいノートを書く前に消す。理由: 古いノートには後で覆った事実が残っていて、正のドキュメント（docs/dev）より先に読まれると誤った前提で開発が進む。過去のノートは履歴に残る。（#98・#100。理由を CLAUDE.md から移した。#190）→ 取りやめた。過去のノートは消さない（2026-09-26、ユーザーの判断）
...
- SQL の本文を書き換えない約束の唯一の例外として、`TRINO_CATALOG_MAP` の別名を引用符付きの修飾名に当てる置換（`catalog.rs`）を入れた。理由: Trino には `/` を含むカタログ名を作れず、S3 Tables の修飾名はほかに Trino へ通す方法が無い。汎用ツールとして入れ、条件は広げない。引用符の無い名前や大文字小文字の違う名前は、Trino 側のカタログ名を合わせる回避策（`docs/configuration.md`・`docs/caveats.md`）で受ける。（2026-09-15。理由を CLAUDE.md から移した。#190）→ S3 Tables の Context の CTAS の無引用の 1 部目 `awsdatacatalog` に限って、`AwsDataCatalog` の別名の Trino 名に差し替える（#232。下の #232 の項）。本物がそう扱うと実測した書き換え（#235）で、別名置換の条件そのものは広げていない
- 「書き換えない」の絶対禁止は、`athena-sql` crate ができたのを機に「本物の Athena がそう扱うと実測した場合だけ書き換える」に緩めた（CLAUDE.md の開発上の約束）。上の 4 つの理由のうち crate で変わったのは (3) だけ（字句と文の頭・句は読めるようになった。正しさを判定しないことは変わらない）。(1) は実装能力でなく目的の話なので残し、方言の変換（Trino で通すための書き換え）は今までどおりしない。(2) は crate の「範囲の差し替えだけ」で守り、(4) は書き換えた SQL を実行の中だけに置くことで守る。絶対禁止のままだと、#227 のように本物どおりに作るには書き換えが要る場面で本物と違う挙動を選ぶことになる。別名置換を「唯一の例外」とする書き方はやめたが、置換の条件を広げない判断は変えない。書き換えの条件を足したら、CLAUDE.md の不変条件の一覧と `docs/configuration.md` の "not rewritten" の約束を同じ PR で直す。（#235、2026-09-26、ユーザーの判断）
- 文の前後の空白と `;` は StartQueryExecution の入口で落とし、落とした文を `Store` の `Query` にも入れる（(4) の例外）。理由: 本物は `;` で区切った片のうち空白だけでない 1 つの前後の空白を落とした文を「文」とし、`GetQueryExecution` の `Query`・構文エラーの位置と文言・文を引用する文言（2 catalogs）をすべてその文で決める（2026-09-26 実測、[measurements/statements.md](measurements/statements.md) の #240 の節）。実行の中だけで切る案（`Query` は受け取ったまま、`Cursor::at_end` に頼る 2 判定を `;` 対応に直す）は、本物の `Query` と食い違ううえ、判定ごとに `;` を読む処理が散るので採らなかった。`;` の無い文の前後の空白も落とす（本物の `Query` がそう）。冪等の指紋だけは受け取ったままの文で作る（本物の比較がそう）。落とすのは前後だけなので、残った文は受け取った SQL の部分文字列のまま（(2) の「範囲の差し替えだけ」を守る）。（#240、2026-09-26、ユーザーの判断）
- DESCRIBE などの修飾を本物どおり落とす（(4) の 2 つ目の例外）。本物は DESCRIBE・DESC・SHOW COLUMNS・SHOW CREATE TABLE・SHOW TABLES IN・ALTER TABLE・DROP TABLE の `awsdatacatalog.` を Context のカタログとして扱って落とし、表への DESCRIBE は DB も落として Context の Database を修飾の DB にする（2026-09-26 実測 ROUND=8、[measurements/statements.md](measurements/statements.md) の #242 の節）。落とす位置は開始時の確認（entity_check）より前: 後にすると、無引用の `awsdatacatalog` を Trino のカタログ名として引いて 3 部の DESCRIBE・SHOW COLUMNS を開始時に弾いてしまう。構文チェックと開始時の文言の判定（quoted_names・unquoted_ddl）には受け取った文を渡す（文言の位置と input は受け取った文で実測しているため）。ビューは Query を受け取った文のまま返すので、Store に表示用の値（`Execution::reported`）を持つ。条件は測った範囲（Context の Catalog が AwsDataCatalog か省略、無引用、部品の数がちょうど）に絞り、4 部以上は本物が名前の形で弾く判定に任せる。（#242、2026-09-26、範囲・ビューの表示・ブロックコメントの FAILED はユーザーの判断）
- 本物の Hive のパーサがブロックコメントで失敗させる文は、文言と位置の規則を `comment_parse_error` の字句の判定 1 か所に閉じ、表の種類（Iceberg は成功）で使うかを呼び出し側が決める。Trino に文が無い MSCK・ADD COLUMNS は構文チェックの前で判定だけ行い、失敗の取り付けは構文チェックの後の開始時の判定（`drop_catalog`・`entity_check`・`quoted_names`・`unquoted_ddl`）の後にそろえる（本物もそれらを実行より先に行うので、構文チェックを飛ばしても開始時の弾き方は変えない）。表の有無と形式は `entity_check` の問い合わせの解釈 1 つを共有する（「無い表」と「判定できない」を区別し、判定できなければ今までどおり送る）。ALTER TABLE の RENAME TO・DROP COLUMN は ErrorMessage が StateChangeReason と別なので `Failure::error_message` に持たせる。範囲（ALTER の 3 動作、Iceberg 表への MSCK をコメントによらず 2/1200）はユーザーの判断（#244、2026-09-26）
- 設計・範囲の選択肢を人間に示すときは、実測に近い方を推奨にする（CLAUDE.md の開発上の約束に足した）。#242 で工数の小さい「DESCRIBE だけ」を推奨にし、本物に近い「カタログ落ちも含める」を 2 番目にしていたのを、ユーザーに指摘された。（#242、2026-09-26、ユーザーの判断）
- SQL の字句走査の道具（`skip_leading_trivia`、`skip_quoted`、`comment_end` など）と文の頭・句を読む API（`Cursor`、`QualifiedName`、`words_iter`）は内部 crate `athena-sql`（`crates/athena-sql/src/` の `trivia.rs`・`words.rs`・`name.rs`・`cursor.rs`。`lib.rs` は `mod` と `pub use` だけ）に集める。athena-local の中に字句処理のモジュール（`lexer.rs` など）を作らず、リテラル・識別子・修飾名の読み方も athena-local に書かない。判定の規則（どの語の並びがどの分類か）は athena-local に残す。理由: 字句処理が 2 か所に散る。区切りは全部 ASCII なので UTF-8 の境界は壊れない。（#17、2026-09-18、#49、2026-09-22。#194、2026-09-25 で「`src/catalog.rs` に集める」を変更。#195、2026-09-25 で crate をモジュールに分け、文の頭・句の API を足した）
- キーワードの照合は `athena-sql` の `Cursor::keyword` 1 つだけ（先頭のトリビアを自分で読み飛ばし、続きが識別子の文字なら一致させない）。同じ規則の別定義は作らない（TRUE／FALSE のようなリテラルも `keyword` で読む）。ラッパーを残して呼び出し元を変えずに済ませる案は、前例になるので採らない（#195 の途中で置いた `skip_keyword` の橋渡しは、呼び出し元が全部 `Cursor` に寄った時点で消した）。（#49、2026-09-22。#44 の「2 版を統合しない」を変更。#195、2026-09-25 で「`skip_keyword` 1 つ」を変更）
- 識別子の文字（ASCII の英数字と `_`）の判定は `athena-sql::trivia::is_identifier_byte` 1 つに集め、`Cursor::keyword`・識別子の継続の読み取り、修飾名の無引用の名前部分（`name.rs`）、`words` の語の境目（`words.rs`）が共有する。同じ規則の別定義は作らない（計画レビューで、この判定が `cursor.rs`・`name.rs` の 2 か所に散っている指摘を受け、`words.rs` を足す前に 1 つへ統合した）。（#200、2026-09-25）
- `words`（`athena_sql::words_iter`）は空白・コメントに加えて、識別子の文字と ASCII の記号（引用符の区間を含む）の境目でも語を切る。制御文字（`\x0B`・`\x0C`）と非 ASCII の空白（`U+3000`・`U+00A0`）は境目にしない: Trino がこれらを構文エラーにするので athena-local では分類まで届かず、境目にしても観測できる差が無いまま固定表の期待値だけが理由なく動く（計画レビューで指摘）。本物はキーワードの直後に空白が無くても空白ありの形と同じ `StatementType`／`SubstatementType`／結果ファイルの置き場所／Content-Type に分類するので（2026-09-25 実測。[measurements/statements.md](measurements/statements.md)）、この規則で空白の無い形を空白ありの形と同じ語に割る。引用符付きの名前を取る `DESCRIBE`・`DESC`・`SHOW CREATE TABLE`・`ALTER TABLE ... ADD COLUMNS`・`DROP TABLE` の 5 種は、本物が空白の有無によらず `StartQueryExecution` の時点で弾く既存の差（語の境界とは無関係）だった。athena-local は #204（2026-09-25）で本物と同じく開始時に弾くようにしたので、この差は解消した（判定は `quoted_names.rs` に別に置き、ここで決めた語の境界の規則とは独立。`OPTIMIZE` は Trino に無いので athena-local はもとから弾いていた）。（#200、2026-09-25）
- `athena-sql` の関数と型は、athena-local から呼ぶものを `pub`（`lib.rs` で `pub use`）、crate の中だけで使う補助（`read_name_part`、`literal_end`、`number_end`）を private にする。athena-local からは `athena_sql::Cursor` のようにフルパスで呼ぶ（#194 の前から `use` で取り込んでいた `content_type.rs`・`iceberg_partitions.rs` と、別名の置換の `catalog.rs` は `use athena_sql::{...}`）。（#49、2026-09-22。#194、2026-09-25 で「`catalog.rs` の `pub(crate)`、`crate::catalog::foo()` のフルパス」を変更。#195、2026-09-25 で補助の例を変更）
- 文の種類の判定箇所を増やさない。`ResultFile::of`（ファイル名）と `statement_type`／`substatement_type` は同じ前処理（`athena_sql::words` と、先頭の `(` を剥がす `results::strip_open_parens`）と同じ判定を使い、新しい判定が要るときは `classification` の関数を再利用する。（#6、2026-09-17、#17、2026-09-18、#39、2026-09-20、#52、2026-09-22。#195、2026-09-25 で剥がし方を 1 つの関数にした）
- キーワードの間のコメント（`/* c */`、`-- c`）は空白として読み飛ばす。引用符（リテラル）の中の `--`／`/*` はコメントと読まない。字句処理は必ずリテラルを `skip_quoted` で読んでからコメントを見る。（#52、2026-09-22、#70、2026-09-23）
- `is_create_table_as` に渡す語列の非対称（`classification` 側は先頭の `(` を除いた後、`results.rs` 側は括弧付き）は変えない。代わりに `is_create_table_as` が `strip_open_parens` で語ごとに先頭の `(` を外してから `AS` の次の語を見るので、どちらの語列でも同じ答えになる（`AS(` は #200 から `athena_sql::words` が `AS` と `(` の語に分けるようになったので、`is_create_table_as` 自身の `AS(` の特別扱いは消した）。CTAS とみなす `AS` の次の語は `SELECT`／`WITH`／`VALUES`／`TABLE` で、本物は括弧の有無・`AS(`・`VALUES`・`TABLE` のどれも、実行が失敗しても `CREATE_TABLE_AS_SELECT` と `tables/<id>` にする（2026-09-25 実測。[measurements/statements.md](measurements/statements.md)）。（#17、2026-09-18。#195、2026-09-25 で「吸収している」を訂正。#199、2026-09-25 で割れていた判定を本物に揃えた。#200、2026-09-25 で `AS(` の特別扱いを消した）
- 修飾名の読み取りは `athena-sql` の `Cursor::qualified_name` 1 つで、名前部分が 1 つでも読めなければ（`.t`、`cat..t`、先頭が名前の文字でない）None にして位置を進めない。位置だけを進める `skip_qualified_name`（空の名前部分を許す）と中身を取り出す `target_table` の `parse_qualified_name`（空なら None）の 2 実装を、厳しい方に揃えて統合した。変わるのは `ALTER TABLE cat..t …` のような入力の分類だけで、本物の Trino が構文エラー（400）にして実行を作らないので観測できない（compose の Trino 482 で確認）。（#195、2026-09-25。#44 の「用途が違うので無理に共通化しない」を変更）
- カタログ別名専用の `next_is_dot` は、ほかの字句関数とループが数行重複しても書き直さない。理由: Trino の実機で検証済みの機能に挙動変化のリスクを持ち込まない。（#17、2026-09-18）
- `ALTER TABLE` の分類は全文走査ではなく位置固定（`Cursor` でキーワードと修飾名を順に読み進める）。理由: 全文走査は `SET TBLPROPERTIES ('comment'='...add column...')` を `ALTER_TABLE_ADD_COLUMN` と誤判定する。（#39、2026-09-20。#195 で `Cursor` に）
- `ADD COLUMN`／`ADD COLUMNS` は `COLUMNS` → `COLUMN` の順に `Cursor::keyword` で試して両方の綴りを受ける（Trino の綴りで書けば動く。実装は #49 で `starts_with("COLUMN")` から `skip_keyword` に変わっていたが記述が古かった。#195 で訂正）。`DROP` は単数形の `COLUMN` だけ（本物は複数形を構文エラーにする）。`REPLACE` は複数形の `COLUMNS` だけで、測っていない単数形は分類しない。（#39、2026-09-20、#43、2026-09-21）
...
- 本物が `StartQueryExecution` の時点で弾く、引用符付きの名前を取る DDL 系の文の判定は、新モジュール `operation/quoted_names.rs`（純関数、athena-local 側）に置き、`classification`（`StatementType`／`SubstatementType` の判定）とは別の関心として扱う。SQL は書き換えず読むだけで、`athena-sql` crate には足さない（crate は SQL が正しいかどうかを判定しない。上の「SQL の本文を書き換えるのは実測した場合だけ」・「SQL の内部 crate」の決定と両立させる）。キーワードの並びは `target_table::table_name_start` を `pub(super)` にして共有し、同じ判定を作り直さない。（#204、2026-09-25）
- `quoted_names::rejection` は構文チェック（`Trino::syntax_error`）の**直後**に呼ぶ。計画では構文チェックの直前だったが、実測で覆した: 本物は Trino も構文エラーにする形（`ALTER TABLE "t" ADD COLUMNS`・`SET TBLPROPERTIES`・`MSCK` など）には Trino 自身の文言をそのまま返しており、athena-local の文言より Trino の文言が勝つ。構文として正しい形だけがこの判定の対象になる。（#204、2026-09-25）
- 弾くのは 2 ラウンドの実測で確かめた形だけにし、実測していない形（3 部の `ALTER TABLE` 名で 2 番目だけ引用符付き、4 部以上の名前、`ALTER TABLE IF EXISTS ...` に引用符付きの名前、`SHOW TABLES IN`／`CREATE TABLE` の 2 部以上の名前、`DESCRIBE`／`DESC` の引用符付きの部分に非 ASCII を含む形）は今までどおり実行する。文言を実測できない形を推測で埋めない、というユーザーの判断による。（#204、2026-09-25、ユーザーの判断 D2）→ #207 のラウンド 3・4 の実測で、3 部の `ALTER TABLE` 名で 2 番目だけ引用符付きも一般の規則で弾くと判明し、非 ASCII を除外していた理由（`DESCRIBE "日本"` が構文の文言でなく Entity Not Found になる）は非 ASCII ではなく対象の不在で決まると分かった。存在の確認（下の #207 の決定）を実装したので、非 ASCII の除外は撤回し、DESCRIBE・SHOW COLUMNS を含むすべての文で一般の規則に含めた。4 部以上・`ALTER TABLE IF EXISTS`・`SHOW TABLES IN`／`CREATE TABLE` の 2 部以上も同じラウンドで大半を実測して一致条件を広げ、実測できなかった一部（`SHOW CREATE TABLE`／`SHOW TABLES IN`／`CREATE TABLE` の 4 部以上、`SHOW TABLES IN` の 3 部）だけ今までどおり実行する（#207、2026-09-25、ユーザーの判断）。 → #212 のラウンド 5 で残りを測って一致条件を広げた（SHOW CREATE TABLE の 4 部以上は DESCRIBE と同じ `Invalid table name`、CREATE TABLE の 4 部以上は DROP と同じく引用符付きの部分の位置で分かれ、`SHOW TABLES IN` は 3 部以上を 2 つ目の `.` で弾く）。4 部の名前で本物が測っていない位置（CREATE TABLE の 1・3 部目、SHOW TABLES IN の 1 部目）に引用符付きの部分がある形は、3 部の名前で同じ位置を測ってあり、4 部の 2 部目が 3 部と同じ文言だったので、4 部目を読む前に文言が決まるとみて 3 部の規則を当てる（`CREATE TABLE IF NOT EXISTS` も同じ。計画攻撃で、コメントと docs が「IF NOT EXISTS の 4 部以上は実行する」と書きながらコードは弾いている食い違いを指摘され、弾く側に揃えた）。CTAS の 4 部以上と、引用符付きの部分が 3 部目までに無い IF NOT EXISTS の 4 部以上（3 つ目の `.` の文言）は実測していないので今までどおり実行する（#212、2026-09-25）。
- DESCRIBE・SHOW COLUMNS の対象の存在の確認は、新しい SQL を作らず既存の `table_format::probe_sql`（`pub(super)` に上げただけ）をそのまま流用する。1 回の問い合わせで「カタログが Trino に無い」「テーブル（かスキーマ）が無い」「ビュー」の 3 つが決まり（手元の Trino で確認）、新しい一致台帳（テストの SQL の写し）も増えない。応答の読み分け（列名 `_col0`／`_col1`、1 行 2 列）は新モジュール `entity_check.rs` に置く。（#207、2026-09-25）
- 存在の確認に渡すスキーマ・テーブル名は小文字にする。本物は大文字の無引用の名前でも実在のテーブルを見つけるが、Trino のカタログは小文字で持つため（手元の Trino で確認）。カタログは `config.trino_catalog` で別名を解決した後の値を使う。（#207、2026-09-25）→ カタログ名も小文字にする。Trino は引用符付きのカタログ名も大文字小文字を区別せずに引くので、`"Hive".db.t` を大文字のまま探すとカタログ不在と取り違える（#207 の独立レビュー、2026-09-25）
- 存在の確認が決められないとき（Trino への問い合わせが失敗する、応答が期待した形（列名・列数・行数）でない、カタログ・スキーマが `parse_target_table` で決まらない）は弾かずに今までどおりの経路（`quoted_names` へ続ける）に倒す。安全側は「対象がある前提で今までどおり動かす」方向で、存在を確認できたときだけ新しく弾く。（#207、2026-09-25）
- カタログ不在の `DATACATALOG_NOT_FOUND` は、名前に 3 部書いてカタログまで指定した形だけで返す。既定・文脈のカタログが Trino に無いとき（名前が 1〜2 部で省略した既定のカタログが無い場合）は未実測なので、存在の確認では判定せず今までどおり実行する。（#207、2026-09-25）→ #214 で測った: 本物は実在しない Context のカタログでもメタデータの文を既定のカタログで解決する。athena-local は `context_catalog::resolve` で先に差し替えてから存在の確認に渡すので、ここに来るのは差し替え先も無いときだけ（下の #214 の決定）。
- 実在しない QueryExecutionContext の Catalog は、本物が既定のカタログで成功させたメタデータの文（`DESCRIBE_TABLE`・`SHOW_COLUMNS`・`SHOW_TABLES`・`SHOW_DATABASES`・`SHOW_CREATE_TABLE`・`DROP_TABLE`）だけ差し替える。表を読む SELECT・EXPLAIN・CTAS は本物も失敗させたので差し替えず、Trino の `CATALOG_NOT_FOUND` を ErrorType 1006 にする（文言の `not found` ↔ `does not exist` と、本物の CTAS の 1300 は変換しない）。差し替え先は `TRINO_CATALOG_MAP` の `AwsDataCatalog` の別名（キーは大文字小文字を区別しない）、無ければ `TRINO_CATALOG`（ユーザーの判断。本物の既定が AwsDataCatalog なので、その名前に忠実にする）。有無は `system.metadata.catalogs` に小文字で問い合わせて決め、Context にカタログがあり別名でもないメタデータの文のときだけ問い合わせる。問い合わせが失敗したり形が違ったりしたら今までどおり。判定は `context_catalog::resolve` 1 か所に置き、開始時（`entity_check` の前）と実行時（`run`）の 2 回呼ぶ（#207 の「2 回飛ぶのは受け入れる」と同じ理由で Store に持ち越さない）。`resolve` は差し替えたとき Trino 側の名前を返す: 別名のキーを返すと `entity_check` の「別名なら問い合わせない」に当たって、実在しない表を開始時に弾けない（計画レビュー）。Store・Fingerprint・GetQueryExecution の Catalog は受け取った値のまま。（#214、2026-09-25）→ #217 で残りを測った: CTAS でない `CREATE TABLE`・`ALTER TABLE ... ADD COLUMNS`・`CREATE VIEW`・`SHOW CREATE VIEW`・`DROP VIEW`・`CREATE`／`DROP DATABASE`（`SCHEMA`）も本物は既定のカタログで成功させたので、対象に足した（`CREATE_TABLE`・`ALTER_TABLE_ADD_COLUMN`・`CREATE_VIEW`・`SHOW_CREATE_VIEW`・`DROP_VIEW`・`CREATE_DATABASE`・`DROP_DATABASE`）。本物が成功させても Trino の構文に無い文（Hive の書き方の DDL・`SHOW PARTITIONS`・`MSCK`・`VACUUM`）と、分類が CTAS と同じ `OPTIMIZE`、Trino が実在しないカタログでも成功させる `SHOW FUNCTIONS` は載せない。`CREATE TABLE`・`ADD COLUMN` は athena-local に届く書き方（場所の無い CREATE TABLE・単数の ADD COLUMN）を本物は開始時に弾く（#208）が、文の種類として本物は既定のカタログで解決するので載せる。INSERT（本物は 1300、Trino は 1301）と DELETE・UPDATE・MERGE（どちらも 1301）は差し替えない。（#217、2026-09-26）
- `quoted_names::rejection` は純関数のまま変えず、存在の確認は別モジュール `entity_check.rs` に置いて 3 択の列挙型（`Reject`／`Run`／`Continue`）を返す構成にする。`Reject` は開始時にそのまま弾く、`Run` はビューなので `quoted_names` を呼ばずに実行へ進む、`Continue`（対象外の文・4 部以上・`TRINO_CATALOG_MAP` の別名・実在するテーブル・確かめられなかった場合）は今までどおり `quoted_names` を呼ぶ。4 部以上は `parse_target_table` が None を返し、別名は `config.catalog_map` に一致した時点で問い合わせずに `Continue` を返すので、判定の順序（4 部以上 → 別名 → カタログの有無 → テーブルの有無 → ビュー → 引用符付きの名前）は `entity_check` の中だけで保たれ、`quoted_names` 側の判定は変えずに済む。（#207、2026-09-25）→ 別名で問い合わせずに `Continue` を返すのは、名前の 1 部目に別名を書いた 3 部の名前だけに絞った。`quoted_names` の `Unsupported DDL with 2 catalogs` が効くのはその形だけで、Context や既定から来た別名（pyathena・awswrangler の既定の `AwsDataCatalog=hive` など）を飛ばすと、本物が Entity Not Found で弾く実在しない表を開始時に弾けなかった。Context から来た別名は、種類（AwsDataCatalog か S3 Tables か）で分けずに Trino 側の名前で存在を確かめる。S3 Tables の Context での本物の扱いは未実測（unmeasured.md）。（#216、2026-09-26、ユーザーの判断）
- 開始時（`entity_check`）と実行時（`format_probe`）で同じ探索が 2 回 Trino に飛ぶのは受け入れる。結果を `Store` へ持ち越して 1 回に減らす案は `Store` の構造変更になるので見送った。（#207、2026-09-25）
- 本物が `StartQueryExecution` の時点で弾く、無引用の `ALTER TABLE`（`IF EXISTS`・単数の `ADD COLUMN`・Trino だけにある 6 形）の判定は、`quoted_names.rs` に足さず新モジュール `operation/unquoted_ddl.rs` に分ける。`quoted_names` の関心は「名前のどこかが引用符付き」で決まる文言、`unquoted_ddl` の関心は「無引用の名前の後ろの語」で決まる文言と、担当する軸が違う（#204・#207 で決めた粒度をそのまま延ばす）。呼び出しは `start_checks.rs` で `quoted_names::rejection(...).or_else(|| unquoted_ddl::rejection(...))` と、**引用符付きの名前の文言を先**にする: 4 部以上の無引用の `ALTER TABLE` 名（例: `a.b.c.d ADD COLUMN m int`）は `quoted_names` が 3 つ目の `.` で先に文言を決める（本物も名前を丸ごと読んでから後ろの語を見るため）ので、順序が逆だと `unquoted_ddl` の ADD COLUMN の文言が勝ってしまう（ミューテーション確認で固定）。`ALTER TABLE IF EXISTS ...` は `quoted_names` 側では判定せず（`if_follows` が真なら常に `None`）、引用符の有無によらず `unquoted_ddl` が丸ごと引き取る: IF EXISTS の直後で本物の構文が壊れるので、名前が引用符付きかどうかは文言に影響しない。`if_follows`（`ALTER`／`CREATE` の直後に `TABLE IF` が続くか）は `quoted_names.rs` の中に閉じたクロージャだったのを `target_table::if_follows` に出し、両モジュールで共有する。（#208、2026-09-26）
- Trino 形の文言（`mismatched input 'ALTER'. Expecting: ...`・`missing 'TO' at 'COLUMN'`・`mismatched input 'EXISTS' expecting {<EOF>, '.'}`）の `'...'` は、元の SQL の綴りをそのまま入れる（小文字での実測は無い）。ANTLR が生成するエラーはトークンの原文を出すので、`quoted_names.rs` の `no viable alternative` と同じ理由で決め打ちにしない。（#208、2026-09-26）
- 弾く範囲はユーザーの判断で 2 回広げた: (1) 単数形の `ADD COLUMN` を弾く（D2。推奨は「弾かない」だったが、本物自身がこの綴りを `StartQueryExecution` で弾く以上、athena-local が実行できても両エンジンが受け付ける書き方が無いため、弾く側に揃えた。結果として `docs/ddl.md`・`docs/caveats.md` が案内していた「athena-local を動かす専用の書き方」は無くなった）。(2) `IF EXISTS` の無い Trino だけの ALTER 5 形（`RENAME COLUMN`・`SET PROPERTIES`・`SET AUTHORIZATION`・`EXECUTE`・`ALTER COLUMN`）も同じ issue でまとめて弾く（D5。推奨は「別 issue に起票」だったが、実測済みの文言がまとまってあり、`ALTER TABLE ... EXECUTE optimize` を弾くと Iceberg maintenance の案内（`docs/caveats.md`）も同時に撤回が要るため、1 つの PR にまとめた）。`DROP COLUMN IF EXISTS`（IF EXISTS 無しの `ALTER TABLE` に対する列側の `IF EXISTS`）も同じラウンドで実測できたので同じ issue に含めた。`ALTER TABLE t DROP COLUMN m`（`IF EXISTS` 無し）は本物も実行でき実行時に失敗するだけなので弾かない（対照 b19）。（#208、2026-09-26、ユーザーの判断 D2・D5）
- CTAS でない無引用の `CREATE TABLE` の判定は、`unquoted_ddl.rs` のサブモジュール `operation/unquoted_ddl/create_table.rs` に置く（ALTER TABLE とは読む対象が違うので関数は分けるが、`start_checks.rs` から見た呼び出し口は 1 つのまま: `unquoted_ddl::rejection` が ALTER TABLE でなければ `create_table::rejection` に回す）。列の並びは、名前（無引用の識別子）→ 型（識別子）→ 型の後ろ、の順に読み、`quoted_names.rs`・`unquoted_ddl.rs` と同じ `no_viable_alternative`／位置の規則を再利用する（D6。#208、2026-09-26、ユーザーの判断は「実測した形を全部弾く」）
- ラウンド 2 の実測結果を受けて、型名を識別子の種類で区別しない一般化に改めた（D7。統括役の判断、ユーザーが「追加で実測する」を選び後押しした）。理由は 3 つ: (1) 型名は構文的に区別されない（`row`・`array`・`map`・存在しない `foo` のどれでも同じ「識別子」としてしか読めない）ので、名前ごとに専用の分岐を書くと未実測の型名を推測で埋めることになる。(2) 実測（型名 21 種、`row`／`array`／`map`／`ROW`／`varchar(x)` の 5 変種）が実際に同じ規則で説明できた。(3) 一般化のほうが分岐が減り、`docs/caveats.md` の説明も「型名は識別子ならどれでも」の 1 文で済む。同じ理由で `LIKE` も「列の名前」という汎用の読み方に含め、専用の分岐を置かない（`(LIKE u)` は 1 列の名前が `u` の表として読み、`(LIKE db.u)` は型として読んだ `db` の後ろの `.` で NV になる。本物の文言と一致した）
- 名前が 4 部以上か、引用符付きの部分を含むかの判定は、`quoted_names.rs` の担当と実測の範囲に合わせて外側で弾く（`create_table::rejection` の入り口で `name.parts.len() >= 4` と `starts_with('"')` を見るだけ）。理由: 4 部以上の無引用の名前と、引用符付きの列名は #208 で実測していない（`docs/dev/unmeasured.md`）。3 部以下で引用符付きの部分がある名前は `quoted_names.rs` が `unquoted_ddl::rejection` より先に文言を決めるので（`start_checks.rs` の `or_else` の順序）、`create_table.rs` 側の引用符チェックは主に「`quoted_names` が判定を諦める 4 部以上」を安全側（未実測は実行する）に倒すための保険（#208、2026-09-26）
- 無引用の 3 部の名前の場所の無い `CREATE TABLE` の 1 部目のカタログの実在は、`DESCRIBE`（`entity_check`）と同じく「Trino にある」で決める（`awsdatacatalog` は大文字小文字によらず問い合わせずに実在、それ以外は `TRINO_CATALOG_MAP` のキーを大文字小文字によらず当ててから `catalog_exists_sql`）。別名の表だけで決める案（Trino に問い合わせない）は、Trino にだけあるカタログ（本物は未実測）を推測で `DATACATALOG_NOT_FOUND` にするので採らなかった。問い合わせが確かめられなければ今までどおり No location。判定は `entity_check`・`TargetStatement` を広げず新モジュール `create_table_catalog.rs` に置いた（`TargetStatement` は `format_probe` と共有で、CREATE TABLE を足すと実行時の問い合わせが増える）。問い合わせは `unquoted_ddl::rejection` が No location を返したときだけ走らせ、NV・2 catalogs・引用符付き・4 部以上の文の Trino への要求は増やさない。（#227、2026-09-26、ユーザーの判断）
- S3 Tables の Context で `AwsDataCatalog.<名前空間>.<表>`（小文字ちょうどでない `awsdatacatalog`）は、本物は 1 部目を無視して名前空間に作る。本物どおりに作るには SQL の 1 部目を落とす書き換えが要り、当時の「書き換えない」約束に反するので（#235 で条件付きに緩めた。1 部目を落とす書き換えは再検討できる）、名前空間が無いときだけ Trino に送らずに本物と同じ FAILED（`Failure::cannot_find_table`、結果ファイルも `.metadata` も置かない）にし、あるときは No location のまま。3 案（No location のまま・無いときだけ FAILED・Trino にそのまま渡す）のうち実測に一番近い。FAILED は開始時に `Submission::immediate_failure` として持たせ、`spawn_query` が `mark_running` の後で `finish` する（取り消しとの競合は既存のガードに任せる）。1 部目が Trino にある他のカタログのときは未実測なので名前空間を見ない。2 部の `<db>.<t>` は #231、別カタログの名前の CTAS は #232 に分けた。（#227、2026-09-26、ユーザーの判断）
- S3 Tables の Context の無引用の 2 部の `<名前空間>.<表>` も、名前空間が無ければ 3 部と同じ FAILED にする（本物は i2・j12 で同じ FAILED）。あれば今までどおり Trino に送る（本物は作る）。1 部の名前（名前空間は Context の Database）は測っていないので問い合わせない。#237 で 3 部の 1 部目を落とすなら、この 2 部の確認に寄せられる（`create_table_catalog::namespace_missing` を共有している）。（#231、2026-09-26）
- S3 Tables の Context の `AwsDataCatalog.<名前空間>.<表>`（小文字ちょうどでない `awsdatacatalog`）は、名前空間があれば 1 部目から 2 部目の直前まで（`.`・空白・コメント）を文字数ぶんの空白にした文を Trino に送る（#235 で緩めた「本物がそう扱うと実測した場合だけ」の書き換え。j1・j4）。落とすのでなく空白にするのは、Trino のエラー位置を受け取った文に合わせるため（改行は残す）。`Query` は `Reported` で受け取ったまま返す。名前空間の有無が確かめられないとき（`SCHEMA_NOT_FOUND` 以外の失敗）も書き換えて送り、2 部の名前（#231）と揃える（No location は書き換えを禁じていた時期の名残）。（#237、2026-09-26）
- S3 Tables の Context の CTAS で、無引用の 3 部の名前の 1 部目が大文字小文字によらず `awsdatacatalog` なら、本物は CTAS でない `CREATE TABLE`（2 部目を S3 Tables の名前空間として引く。#227・#237）と逆に、2 部目を Glue の DB として引いた（2026-09-26 実測 i12 は小文字で DB があり SUCCEEDED、j13 は `AwsDataCatalog` で DB が無く `Database <ns> not found. ...` の FAILED）。athena-local は `TRINO_CATALOG_MAP` の `AwsDataCatalog` の Trino 名（キーは大文字小文字によらず当てる。別名が無ければ `awsdatacatalog`）で DB を `schema_probe_sql` で確かめ、無ければ Trino に送らずに本物と同じ理由（場所は結果の置き場所の `uri()`）・2/1301 の FAILED、あれば（確かめられなくても）1 部目だけを `catalog::replacement`（引用符付きの Trino 名と空白。引用符付きの別名置換と同じ形で、Trino 名が長ければ桁がずれる）に差し替えて送る。Trino 名が `awsdatacatalog` なら差し替えない（Trino は無引用の名前を小文字で引くので同じ）。`Query` は `Reported` で受け取ったまま。判定は `create_table_catalog::ctas` に置き、`start_checks.rs` は呼び出し 1 ブロックだけ（開始時の判定は #245 で `execution.rs` から `start_checks.rs` に分けた）。
...
- Iceberg のテーブルの `DESCRIBE` の `# Partition spec:` の下の行は、Trino の `DESCRIBE` から取れないので、対象の Trino の `SHOW CREATE TABLE` の `partitioning = ARRAY[...]` から作る（`operation::iceberg_partitions`。問い合わせは Iceberg × DESCRIBE のときだけ 1 本増える）。名前は元の SQL の範囲を `athena_sql::Cursor::qualified_name`（`QualifiedName.text`）で切り出して `alias_qualified_names` を当て、手で修飾名を組まない。失敗は握ってパーティション行を出さない（本体は SUCCEEDED のまま）。測っていない変換は行を出さない。（#173、2026-09-24、ユーザーの判断 D3）
