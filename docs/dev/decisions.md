...
- 名前が 4 部以上か、引用符付きの部分を含むかの判定は、`quoted_names.rs` の担当と実測の範囲に合わせて外側で弾く（`create_table::rejection` の入り口で `name.parts.len() >= 4` と `starts_with('"')` を見るだけ）。理由: 4 部以上の無引用の名前と、引用符付きの列名は #208 で実測していない（`docs/dev/unmeasured.md`）。3 部以下で引用符付きの部分がある名前は `quoted_names.rs` が `unquoted_ddl::rejection` より先に文言を決めるので（`execution.rs` の `or_else` の順序）、`create_table.rs` 側の引用符チェックは主に「`quoted_names` が判定を諦める 4 部以上」を安全側（未実測は実行する）に倒すための保険（#208、2026-09-26）
