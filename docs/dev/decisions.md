...
- SQL の字句走査の道具（`skip_leading_trivia`、`skip_quoted`、`comment_end`、`skip_keyword` など）は内部 crate `athena-sql`（`crates/athena-sql/src/lib.rs`）に集める。athena-local の中に字句処理のモジュール（`lexer.rs` など）を作らない。理由: 字句処理が 2 か所に散る。区切りは全部 ASCII なので UTF-8 の境界は壊れない。（#17、2026-09-18、#49、2026-09-22。#194、2026-09-25 で「`src/catalog.rs` に集める」を変更）
- `skip_keyword` は `athena-sql` に 1 つだけ置く（内部で `skip_leading_trivia` を呼ぶ版）。同名の別定義は作らない。ラッパーを残して呼び出し元を変えずに済ませる案は、前例になるので採らない。（#49、2026-09-22。#44 の「2 版を統合しない」を変更）
- `athena-sql` の字句関数は、athena-local から呼ぶものを `pub`、crate の中だけで使う補助（`skip_name_part`）を private にする。athena-local からは `athena_sql::foo()` のフルパスで呼ぶ（#194 の前から `use` で取り込んでいた `content_type.rs`・`iceberg_partitions.rs` と、別名の置換の `catalog.rs` は `use athena_sql::{...}`）。（#49、2026-09-22。#194、2026-09-25 で「`catalog.rs` の `pub(crate)`、`crate::catalog::foo()` のフルパス」を変更）
...
- Iceberg のテーブルの `DESCRIBE` の `# Partition spec:` の下の行は、Trino の `DESCRIBE` から取れないので、対象の Trino の `SHOW CREATE TABLE` の `partitioning = ARRAY[...]` から作る（`operation::iceberg_partitions`。問い合わせは Iceberg × DESCRIBE のときだけ 1 本増える）。名前は元の SQL の範囲を `athena_sql::skip_qualified_name` で切り出して `alias_qualified_names` を当て、手で修飾名を組まない。失敗は握ってパーティション行を出さない（本体は SUCCEEDED のまま）。測っていない変換は行を出さない。（#173、2026-09-24、ユーザーの判断 D3）
