...
- [ ] S3 Tables の Context の Hive の `CREATE TABLE` で、#229 で測っていない句（表の `COMMENT`、`CLUSTERED BY`、`ROW FORMAT SERDE`・`FIELDS` 以外の `DELIMITED` の句、`TBLPROPERTIES` の 2 組以上、`TBLPROPERTIES` だけ・`STORED AS` だけで `LOCATION` の無い `EXTERNAL`）、`LOCATION` の無い `EXTERNAL` の 2 部の名前と 1 部目が大文字混じりの `AwsDataCatalog` の 3 部、`TRINO_CATALOG_MAP` の別名や Trino にだけあるカタログの 3 部 + `LOCATION`、バッククォートの名前。athena-local は `Table location`／`External keyword` を返さず、Trino の構文チェックに任せる（#229、2026-09-26）
...
- [ ] S3 Tables の Context の 1 部の名前の場所の無い `CREATE TABLE` で、Context の Database の名前空間が無いとき（本物が 2 部・3 部と同じ `Cannot find or access the specified table` の FAILED にするか）。athena-local は名前空間を問い合わせずに Trino に送る。2 部の `IF NOT EXISTS <名前空間>.<表>` も未実測（測ったのは 3 部の `IF NOT EXISTS`（j9）と、2 部の `IF NOT EXISTS` の無い形（j12））で、athena-local は 3 部と同じく名前空間が無ければ FAILED にする（#231、2026-09-26。#251 で測る）
- [ ] S3 Tables の Context の `CREATE TABLE IF NOT EXISTS AwsDataCatalog.<名前空間>.<表>` で名前空間があるとき（測ったのは `IF NOT EXISTS` の無い形（j1・j4）と、無い名前空間の `IF NOT EXISTS`（j9））。athena-local は 1 部目を空白にして作る（#237、2026-09-26。#251 で測る）
- [ ] S3 Tables の Context の CTAS の `awsdatacatalog.<DB>.<表>` の残り: j13 で本物が置いた `.metadata`（81 バイト）の中身と、書かれたデータの有無、理由の `Database <名前>` が書いたとおりか小文字か、`IF NOT EXISTS` の CTAS、大文字混じりで DB がある形と小文字で DB が無い形、既定の Context の同じ形。athena-local は `.metadata` を置かず、名前は書いたとおり、`IF NOT EXISTS` と既定の Context は受け取ったまま送り、綴りでは分けない（#232、2026-09-26。#251 で測る）
...
- [ ] ブロックコメントの ParseException（#244）の周り: コメントが名前の中・名前の後ろ・句の中にあるとき、先頭コメントの中身が 2 文字の記号（`<=`・`!=`・`||`）やエスケープを含む引用符・閉じていない引用符のときの 3 語目、ALTER の REPLACE COLUMNS・SET LOCATION・CHANGE COLUMN・Iceberg 表への RENAME TO と DROP COLUMN の先頭・TABLE の後の位置、S3 Tables・連携カタログの Context、Context の Catalog 省略時、`DESCRIBE` のほかの位置（`DESCRIBE EXTENDED /* c */`）、`MSCK REPAIR TABLE awsdatacatalog.<db>.<t>` の 1 部目（#242 の落とす文に無い。athena-local は落とさずカタログとして引き、無ければ今までどおり構文エラー）、`ALTER /* c */ TABLE awsdatacatalog.<db>.<t> DROP COLUMN` の ErrorMessage の位置を落とす前と後のどちらの文で数えるか（athena-local は落とした後の文。本物が返す Query と同じ）。athena-local は名前の直前までのコメントだけを実測の規則で失敗させ、字句は英数字と `_`・小数・引用符の文字列・記号 1 文字で切る（2026-09-26）。測るのは #257
