...
- [ ] S3 Tables の Context の 1 部の名前の場所の無い `CREATE TABLE` で、Context の Database の名前空間が無いとき（本物が 2 部・3 部と同じ `Cannot find or access the specified table` の FAILED にするか）。athena-local は名前空間を問い合わせずに Trino に送る。2 部の `IF NOT EXISTS <名前空間>.<表>` も未実測（測ったのは 3 部の `IF NOT EXISTS`（j9）と、2 部の `IF NOT EXISTS` の無い形（j12））で、athena-local は 3 部と同じく名前空間が無ければ FAILED にする（#231、2026-09-26）
