...
- [ ] ブロックコメントの ParseException（#244）の周り: コメントが名前の中・名前の後ろ・句の中にあるとき、先頭コメントの中身が 2 文字の記号（`<=`・`!=`・`||`）やエスケープを含む引用符・閉じていない引用符のときの 3 語目、ALTER の REPLACE COLUMNS・SET LOCATION・CHANGE COLUMN・Iceberg 表への RENAME TO と DROP COLUMN の先頭・TABLE の後の位置、S3 Tables・連携カタログの Context、Context の Catalog 省略時、`DESCRIBE` のほかの位置（`DESCRIBE EXTENDED /* c */`）。athena-local は名前の直前までのコメントだけを実測の規則で失敗させ、字句は英数字と `_`・小数・引用符の文字列・記号 1 文字で切る（2026-09-26）
