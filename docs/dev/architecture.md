...
- `crates/athena-sql` — SQL の字句処理と文の頭・句の認識を置く内部 crate（`publish = false`、依存なし）。athena-local からパスで依存する。字句処理の道具は #194 で `catalog.rs` から中身を変えずに移し、athena-local は `athena_sql::foo()` で呼ぶ。文の頭・句の認識はまだ無い（#195。段階は親 #192）。規則は [decisions.md](decisions.md) の「SQL の内部 crate（athena-sql）」。
