...
2. `operation.rs` — 4 つのオペレーションの本体。
   ...
   - `spawn_query` → `run`: `ExecutionParameters` の値ごとに `SELECT (<値>)` を Trino に投げて分類し（`statement::bind`）、`EXECUTE IMMEDIATE '<sql>' USING ...` で実行する。パラメータが無ければ SQL は一切書き換えない（テストで保証している不変条件）。`?` の無い SQL に値が渡されたときのエラーでは、元の SQL で再実行する。
   - `write_result`: 結果 CSV を置いてから `SUCCEEDED` にする（クライアントは SUCCEEDED を見た直後に S3 を読むため）。CSV を置くのは `ResultFile::Csv` で、更新件数が無く、取り消されていないときだけ。
...
- **SQL の本文は書き換えない。** 必要なら全体を包む（`EXECUTE IMMEDIATE`）か、別のクエリを投げる（構文チェックの `PREPARE`、パラメータ分類の `SELECT (<値>)`）。カタログの別名もヘッダにだけ当てる。Athena と Trino の書き方の違い（小数リテラルの型、DDL、`OPTIMIZE` / `VACUUM`）は変換せず、README の Caveats に回避策を書く。字句レベルの置換（SQL 中のカタログ名など）も入れない。利用側に確認したところ（2026-09-14）、athena-local に流す SQL にカタログ名を直書きしたものは無く、書き換え機能は不要との回答だった。
