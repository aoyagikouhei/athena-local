#!/usr/bin/env bash
# issue #35 で作成（tools/ へ移す前の名前は 35-measure-insert-location.sh）
# 本物の Athena で、INSERT の OutputLocation（拡張子なしの `<id>` か、`<id>.csv` か）を
# 実測する。issue #35 のための実測スクリプト。#26 の ctas-iceberg-detection.sh を
...
#
# 使い方:
#   tools/dev.sh OUTPUT=s3://your-bucket/prefix/ DB=your_db bash tools/measure/insert-location.sh
...
#   OUT_DIR      既定 ${DEV_HOST_HOME:-$HOME}/athena-insert-measurements（実名が入るのでリポジトリの外に出す）
...
OUT_DIR=${OUT_DIR:-${DEV_HOST_HOME:-$HOME}/athena-insert-measurements}
