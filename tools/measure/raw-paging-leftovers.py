#!/usr/bin/env python3
# issue #85 で作成（tools/ へ移す前の名前は 85-measure-raw-paging-leftovers.py）
"""#83 で未実測のまま残した 3 点（#85）を、本物の Athena に生 HTTP（SigV4 自前署名）で測る。
署名・送信・保存・要約の関数は同じディレクトリの raw-get-query-results.py から読み込む
...
spec = importlib.util.spec_from_file_location(
    "measure83", os.path.join(HERE, "raw-get-query-results.py")
)
