#!/usr/bin/env bash
# issue #43 で作成（tools/ へ移す前の名前は 43-measure-alter-variants.sh）
# 本物の Athena で、ALTER TABLE の未実測の亜種を実測する。issue #43 のための実測スクリプト。
#
# tools/measure/alter-substatement.sh を雛形にしている（run・
# loc_shape_of・body_shape_of・poll_until_terminal・start_query_retry・preflight・
# trap での後始末・summary の実名マスク・determine_table_format・apply_table_format・
# 出力ファイルの命名規約・summary.tsv の列構成・cleanup-hints.txt はそのまま流用し、
# 測る項目とテーブルの準備だけ差し替えた）。
#
# 出どころ（`gh issue view 43 --repo aoyagikouhei/athena-local` の本文）:
#   #39 の実測（3 ラウンド目）で ALTER TABLE の SubstatementType と結果ファイルを測ったが、
#   測れたのは次の 6 通りだけだった。
#     SET TBLPROPERTIES        hive: 0B/無し           iceberg: FAILED（comment 不可）  ALTER_TABLE_PROPERTIES
#     ADD COLUMNS               hive: 0B/.metadata 38B  iceberg: 0B/無し                 ALTER_TABLE_ADD_COLUMN
#     DROP COLUMN（単数）       hive: FAILED（制約）    iceberg: 0B/無し                 ALTER_TABLE_DROP_COLUMN
#     SET LOCATION               hive: 0B/無し           iceberg: 未実測
#     IF EXISTS ... ADD COLUMNS  両方とも構文エラー
#     RENAME COLUMN               両方とも構文エラー
#   #39 では ALTER 判定を位置固定にし、実測した 4 つの SubstatementType
#   （ALTER_TABLE_ADD_COLUMN・ALTER_TABLE_PROPERTIES・ALTER_TABLE_DROP_COLUMN・
#   ALTER_TABLE_SET_LOCATION）だけを返すようにした。未実測の亜種は None に落ちる
#   （CLAUDE.md の「実測していない振る舞いは推測で埋めない。項目を省く」に従った）。
#   本 issue #43 で、次の未実測のまま残っているものを測る:
#     - ALTER TABLE ... SET LOCATION × Iceberg（issue の筆頭項目）
#     - ALTER TABLE ... ADD PARTITION / DROP PARTITION
#     - ALTER TABLE ... REPLACE COLUMNS
#     - ALTER TABLE ... RENAME TO（SubstatementType が None のままでよいか）
#     - DROP COLUMNS（複数形）を本物が受け付けるか（athena-local は
#       starts_with("COLUMN") で単数・複数の両方を受けてしまう）
#
# 測る項目（すべて GetQueryExecution の StatementType / SubstatementType を必ず記録する。
# これが今回の焦点）。実行順は下の記載順のとおり（列やテーブル名を変える文があるので
# 順序に意味がある）:
#
#   athena_local_probe_43_hive（Hive）に対して:
#     f1  ALTER TABLE <hive> ADD COLUMNS (m int)                       対照（既測の再掲）
#     d1  ALTER TABLE <hive> DROP COLUMNS x                            複数形。Hive の制約で
#                                                                        FAILED のはず。構文エラーか
#                                                                        制約エラーかを reason.txt で見る
#     a1  ALTER TABLE <hive> REPLACE COLUMNS (n int, s string)         未実測
#     e1  ALTER TABLE <hive> SET LOCATION '<OUTPUT>43-loc-hive/'       対照（既測の再掲）
#
#   athena_local_probe_43_ice（Iceberg）に対して（e2 は location を変えるので必ず最後）:
#     f2  ALTER TABLE <ice> ADD COLUMNS (m int)                        対照（既測の再掲）
#     d2  ALTER TABLE <ice> DROP COLUMNS x                             本題。複数形を本物が受けるか
#     g1  ALTER TABLE <ice> DROP COLUMN y                              d2 の対照。単複だけが違う対
#                                                                        （既測: ALTER_TABLE_DROP_COLUMN）
#     a2  ALTER TABLE <ice> REPLACE COLUMNS (n int, s varchar)         未実測
#     c3  ALTER TABLE <ice> ADD PARTITION (p = 'v')                    Iceberg にこの構文があるか
#     e2  ALTER TABLE <ice> SET LOCATION '<OUTPUT>43-loc-ice/'         issue の筆頭項目
#
#   athena_local_probe_43_part（Hive + partitioned_by = ARRAY['p']）に対して:
#     c1  ALTER TABLE <part> ADD PARTITION (p = 'v2') LOCATION '<OUTPUT>43-part-v2/'  未実測
#     c2  ALTER TABLE <part> DROP PARTITION (p = 'v2')                 未実測（c1 が足したものを落とす）
#
#   RENAME 専用（テーブル名が変わるので専用テーブルを使う）:
#     b1  ALTER TABLE athena_local_probe_43_rn_hive RENAME TO athena_local_probe_43_rn_hive2
#     b2  ALTER TABLE athena_local_probe_43_rn_ice  RENAME TO athena_local_probe_43_rn_ice2
#
# 対照（雛形と同じ。preflight 兼）:
#   probe        SHOW TABLES    … <id>.txt / .metadata が置かれるはず（preflight 兼）
#   ctl-select   SELECT 1 AS n  … <id>.csv 8B / .metadata 73B のはず（README・#5 の既知の値）
#
# 準備（CTAS。テーブル名の接頭辞は athena_local_probe_43_。雛形の prep-hive / prep-ice と
# 同じ作り方。Iceberg は location を $OUTPUT の下に置いて DROP でデータも消える形にする）:
#   prep-hive     CREATE TABLE ... AS SELECT 1 AS n, 'x' AS s, 10 AS x                （Hive）
#   prep-ice      CREATE TABLE ... WITH (table_type='ICEBERG', location=..., is_external=false)
#                 AS SELECT 1 AS n, 'x' AS s, 10 AS x, 20 AS y                         （Iceberg。列を
#                 4 個にして d2（x を落とす）と g1（y を落とす）を互いに独立させる）
#   prep-part     CREATE TABLE ... WITH (partitioned_by = ARRAY['p'])
#                 AS SELECT 1 AS n, 'v1' AS p                                          （Hive。パーティ
#                 ション列は SELECT の最後に置く決まりに従う）
#   prep-rn-hive  CREATE TABLE ... AS SELECT 1 AS n                                    （Hive。RENAME 専用）
#   prep-rn-ice   CREATE TABLE ... WITH (table_type='ICEBERG', location=..., is_external=false)
#                 AS SELECT 1 AS n                                                     （Iceberg。RENAME 専用）
#
# テーブルの形式は SHOW CREATE TABLE で裏取りする（DROP される前にしか呼べないので、作った
# 直後に確かめておいた値を、同じテーブルに触れる後続の行に apply_table_format でまとめて
# 書き込む。drop-table-format.sh / alter-substatement.sh と同じやり方）。
# 5 テーブルすべてで確かめる（RENAME 専用の 2 本も、CTAS の WITH 指定が実際に効いたかどうかを
# 独立して裏取りするため）。
#
# 使い方:
#   tools/dev.sh OUTPUT=s3://your-bucket/prefix/ DB=your_db bash tools/measure/alter-variants.sh
...
#   OUT_DIR      既定 ${DEV_HOST_HOME:-$HOME}/athena-alter-variants-measurements（実名が入るのでリポジトリの外）
...
#                本編・対照・準備を含め全項目を skip する（何も作らない）。
#
# ** このスクリプトが本物に対して行う破壊的な操作 **
#   <db>.athena_local_probe_43_hive / _ice / _part / _rn_hive / _rn_ice の 5 テーブルを
#   CTAS で作り、hive/ice/part の 3 本には ALTER TABLE を最大 4〜6 回ずつ（合計最大 14 回）、
#   rn_hive/rn_ice の 2 本には ALTER TABLE ... RENAME TO を 1 回ずつ投げてから、最後に
#   5 テーブルすべてを DROP TABLE で消す（RENAME 済みのものは新しい名前で DROP し、
#   失敗したら元の名前でも試す）。開始前に SHOW TABLES で athena_local_probe_43_* という
#   名前が既に無いことを確かめ、1 件でもあれば（RENAME 先の _rn_hive2 / _rn_ice2 を含む。
#   同じ接頭辞なので 1 回の確認で両方カバーされる）テーブルを作らずに止まる。
#   Hive のテーブル（_hive・_part・_rn_hive）のデータは DROP TABLE では消えない。残った
#   key の一覧を cleanup-hints.txt に書き出すので、消したければそれを見て手で消すこと。
#   Iceberg のテーブル（_ice・_rn_ice）は location を $OUTPUT の下に指定してあり、
...
# summary.txt の冒頭に実測値で出す（trap で発動する後始末のベストエフォート呼び出しだけは
# 数えない。理由は summary.txt に書く。最後の後始末（DROP TABLE）は start_query_retry を
# 通る通常の呼び出しなので数える）。
#
...
#
# alter-substatement.sh から変えたところ（この issue のために足したもの）:
#   - 測る項目を issue #43 の未実測の亜種（SET LOCATION × Iceberg・ADD/DROP PARTITION・
#     REPLACE COLUMNS・RENAME TO・DROP COLUMNS 複数形）に差し替えた。ALTER 判定に関わらない
#     SET TBLPROPERTIES・IF EXISTS・RENAME COLUMN（すでに構文エラーと判明済み）は外した。
#   - 準備するテーブルを 2 本（hive/ice）から 5 本（hive・ice・part・rn_hive・rn_ice）に
#     増やした。part はパーティション操作用、rn_hive/rn_ice は RENAME TO がテーブル名
#     そのものを変えてしまうため専用に分けた。
#   - determine_table_format / apply_table_format を 5 テーブル分に拡張した。
#   - ghost（対象が無い DROP TABLE IF EXISTS）は #39 で測定済みのため今回は含めない。
#   - 最後の後始末（5 テーブルの DROP）を、測る項目とは独立した明示的な手順として追加した
#     （drop_table_final。summary.tsv には行を足さず、cleanup-drops.txt に記録する）。
#     RENAME 済みのテーブルは新しい名前で DROP し、それが失敗したときだけ元の名前でも試す。
#
# 文ごとに次を保存する（取れたものだけ）。
...
#   cleanup-drops.txt          後始末の DROP TABLE の実行結果（label・state のみ。実名は
#                               テーブル名接頭辞だけなので summary.txt には出さない）
#
...
OUT_DIR=${OUT_DIR:-${DEV_HOST_HOME:-$HOME}/athena-alter-variants-measurements}
POLL_TIMEOUT=${POLL_TIMEOUT:-180}
RETRY_MAX=${RETRY_MAX:-4}
RETRY_DELAY=${RETRY_DELAY:-5}
PROBE_DDL=${PROBE_DDL:-1}

PREFIX=athena_local_probe_43
TABLE_HIVE="${PREFIX}_hive"
TABLE_ICE="${PREFIX}_ice"
TABLE_PART="${PREFIX}_part"
TABLE_RN_HIVE="${PREFIX}_rn_hive"
TABLE_RN_ICE="${PREFIX}_rn_ice"
TABLE_RN_HIVE2="${PREFIX}_rn_hive2"
TABLE_RN_ICE2="${PREFIX}_rn_ice2"
# trap での後始末（異常終了時の保険）が対象にするテーブルの接尾辞。RENAME 先の名前
# （rn_hive2 / rn_ice2）も含め、どちらの名前で残っていても DROP TABLE IF EXISTS で拾う。
TABLES="hive ice part rn_hive rn_hive2 rn_ice rn_ice2"
# 本編で測る項目のラベル（preflight・対照・準備を除く）。
LABELS="f1 d1 a1 e1 f2 d2 g1 a2 c3 e2 c1 c2 b1 b2"
# summary.txt の「投げた文」「失敗した項目の理由」を並べる順（実行順と同じ）。
ALL_LABELS="probe ctl-select prep-hive prep-ice prep-part prep-rn-hive prep-rn-ice $LABELS"
...
# run/drop_table_final/determine_table_format はどれも `id=$(... )` とコマンド置換で
# start_query_retry を呼ぶのでサブシェルになり、変数の加算は親に伝わらない。1 回ごとに
# ファイルへ 1 行積み、最後に行数を数える。
...
# テーブル作成に着手していたら、ベストエフォートで DROP も投げる（本編末尾の
# drop_table_final でも消すので、これは異常終了時の保険。正常終了で全テーブルの
# 後始末が終わっていれば、この保険を省いて二重に呼ばない）。
cleanup() {
  rm -f "$RUN_DIR"/.tmp-*
...
# 実名（DB 名・出力先・バケット名）を置換して隠す。prep-ice / c1 / e1 / e2 の SQL には
# location として $OUTPUT がそのまま入るので、DB 名だけでは足りない。
...
# 先に確かめておいた fmt を、同じテーブルに触れる行すべて（作成・ALTER・RENAME）へ
# 後からまとめて書き込む。
apply_table_format() {
...
# 後始末の DROP TABLE を 1 本投げ、終端状態まで待つ。成功したときだけ 0 を返す。
# summary.tsv には行を足さない（測る項目の設計表に無いため）。実行結果は
# cleanup-drops.txt に 1 行追記する（テーブル名の接頭辞だけで実名は含まない）。
drop_table_final() {
  local suffix=$1 name=$2 sql id state
  sql="DROP TABLE IF EXISTS $DB.$name"
  printf '%s\n' "$sql" > "$RUN_DIR/cleanup-$suffix.sql"
  id=$(start_query_retry "cleanup-$suffix" "$sql")
  if [ -z "${id:-}" ]; then
    echo "cleanup-$suffix: 開始できませんでした（$(first_err_line "$RUN_DIR/cleanup-$suffix.start.err")）" >> "$RUN_DIR/cleanup-drops.txt"
    return 1
  fi
  state=$(poll_until_terminal "$id")
  aws athena get-query-execution --region "$REGION" --query-execution-id "$id" \
    > "$RUN_DIR/cleanup-$suffix.execution.json" 2>/dev/null
  echo "cleanup-$suffix: state=$state" >> "$RUN_DIR/cleanup-drops.txt"
  [ "$state" = SUCCEEDED ]
}
...
if fetch "$PROBE_LOC" "$RUN_DIR/tables.txt" "$RUN_DIR/tables.err"; then
  # athena_local_probe_43_* という接頭辞は今回作る 5 テーブルすべて（hive・ice・part・
  # rn_hive・rn_ice）と RENAME 先の名前（rn_hive2・rn_ice2）を含むので、この 1 回の
  # 確認で全部カバーできる（measurement.md 要件 7）。
  if grep -qi "$PREFIX" "$RUN_DIR/tables.txt"; then
    echo
    echo "このデータベースに ${PREFIX}_* という名前のテーブルが既にあります（RENAME 先の"
    echo "${PREFIX}_rn_hive2 / ${PREFIX}_rn_ice2 も同じ接頭辞なので対象に含む）。"
...
if [ "$PROBE_DDL" != 1 ]; then
  for label in ctl-select prep-hive prep-ice prep-part prep-rn-hive prep-rn-ice $LABELS; do
...
# --- 準備: CTAS で ALTER / RENAME の対象テーブルを作る ------------------------

TABLES_ATTEMPTED=1

# Hive の CTAS（Athena の既定）。列を 3 個持たせ、ADD COLUMNS・DROP COLUMNS・
# REPLACE COLUMNS・SET LOCATION を互いに独立して測れるようにする。
run prep-hive "CREATE TABLE $DB.$TABLE_HIVE AS SELECT 1 AS n, 'x' AS s, 10 AS x" keys
PREP_HIVE_OK=$?

# Iceberg の CTAS。列を 4 個持たせ（x は d2 が、y は g1 が落とすので互いに独立）、
# location を結果プレフィックス配下に置いて DROP でデータも消えるようにする。
run prep-ice "CREATE TABLE $DB.$TABLE_ICE WITH (table_type = 'ICEBERG', location = '${OUTPUT}tables-probe-43-ice/', is_external = false) AS SELECT 1 AS n, 'x' AS s, 10 AS x, 20 AS y" keys
PREP_ICE_OK=$?

# パーティション付き Hive の CTAS。パーティション列 p は SELECT の最後に置く。
run prep-part "CREATE TABLE $DB.$TABLE_PART WITH (partitioned_by = ARRAY['p']) AS SELECT 1 AS n, 'v1' AS p" keys
PREP_PART_OK=$?

# RENAME 専用の Hive テーブル。列は RENAME に無関係なので 1 列だけ。
run prep-rn-hive "CREATE TABLE $DB.$TABLE_RN_HIVE AS SELECT 1 AS n" keys
PREP_RN_HIVE_OK=$?

# RENAME 専用の Iceberg テーブル。location を結果プレフィックス配下に置く。
run prep-rn-ice "CREATE TABLE $DB.$TABLE_RN_ICE WITH (table_type = 'ICEBERG', location = '${OUTPUT}tables-probe-43-rn-ice/', is_external = false) AS SELECT 1 AS n" keys
PREP_RN_ICE_OK=$?

# 形式の裏取り（DROP の直前ではなく作った直後にしかできない）。WITH の指定が本当に
# 効いたかどうかを確かめる（RENAME 専用の 2 本も含め、5 テーブス全部で確かめる）。
FMT_HIVE=unknown
FMT_ICE=unknown
FMT_PART=unknown
FMT_RN_HIVE=unknown
...
[ "$PREP_PART_OK" = 0 ] && FMT_PART=$(determine_table_format part "$TABLE_PART")
[ "$PREP_RN_HIVE_OK" = 0 ] && FMT_RN_HIVE=$(determine_table_format rn-hive "$TABLE_RN_HIVE")
[ "$PREP_RN_ICE_OK" = 0 ] && FMT_RN_ICE=$(determine_table_format rn-ice "$TABLE_RN_ICE")

# --- 本編: athena_local_probe_43_hive ----------------------------------------

# f1. ADD COLUMNS（対照。既測: ALTER_TABLE_ADD_COLUMN / .metadata 38B のはず）。
if [ "$PREP_HIVE_OK" = 0 ]; then
  run f1 "ALTER TABLE $DB.$TABLE_HIVE ADD COLUMNS (m int)" keys
else
  skip f1 "$TABLE_HIVE を作れなかったので未測定"
fi

# d1. DROP COLUMNS（複数形）。Hive は制約で FAILED のはず。構文エラーか制約エラーかは
# d1.reason.txt の StateChangeReason / AthenaError で区別する。
if [ "$PREP_HIVE_OK" = 0 ]; then
  run d1 "ALTER TABLE $DB.$TABLE_HIVE DROP COLUMNS x" keys
else
  skip d1 "$TABLE_HIVE を作れなかったので未測定"
fi

# a1. REPLACE COLUMNS（未実測）。d1 の成否に関わらず、列リストを丸ごと置き換えるので
# 独立して実行できる。
if [ "$PREP_HIVE_OK" = 0 ]; then
  run a1 "ALTER TABLE $DB.$TABLE_HIVE REPLACE COLUMNS (n int, s string)" keys
else
  skip a1 "$TABLE_HIVE を作れなかったので未測定"
fi

# e1. SET LOCATION（対照。既測: ALTER_TABLE_SET_LOCATION / 本体 0B・.metadata 無しのはず）。
if [ "$PREP_HIVE_OK" = 0 ]; then
  run e1 "ALTER TABLE $DB.$TABLE_HIVE SET LOCATION '${OUTPUT}43-loc-hive/'" keys
else
  skip e1 "$TABLE_HIVE を作れなかったので未測定"
fi

# --- 本編: athena_local_probe_43_ice（e2 は location を変えるので必ず最後）-----

# f2. ADD COLUMNS（対照。既測: ALTER_TABLE_ADD_COLUMN / .metadata 無しのはず）。
if [ "$PREP_ICE_OK" = 0 ]; then
  run f2 "ALTER TABLE $DB.$TABLE_ICE ADD COLUMNS (m int)" keys
else
  skip f2 "$TABLE_ICE を作れなかったので未測定"
fi

# d2. DROP COLUMNS（複数形）。本題。athena-local は starts_with("COLUMN") で単数・複数の
# 両方を ALTER_TABLE_DROP_COLUMN に判定してしまうが、本物が複数形の構文を受けるかは未確認。
if [ "$PREP_ICE_OK" = 0 ]; then
  run d2 "ALTER TABLE $DB.$TABLE_ICE DROP COLUMNS x" keys
else
  skip d2 "$TABLE_ICE を作れなかったので未測定"
fi

# g1. DROP COLUMN（単数）。d2 の対照。単複だけが違う対（既測: ALTER_TABLE_DROP_COLUMN）。
# d2 が x を落とす（かもしれない）のに対し、g1 は別の列 y を落とすので独立している。
if [ "$PREP_ICE_OK" = 0 ]; then
  run g1 "ALTER TABLE $DB.$TABLE_ICE DROP COLUMN y" keys
else
  skip g1 "$TABLE_ICE を作れなかったので未測定"
fi

# a2. REPLACE COLUMNS（未実測）。d2・g1 の成否に関わらず列リストを丸ごと置き換える。
if [ "$PREP_ICE_OK" = 0 ]; then
  run a2 "ALTER TABLE $DB.$TABLE_ICE REPLACE COLUMNS (n int, s varchar)" keys
else
  skip a2 "$TABLE_ICE を作れなかったので未測定"
fi

# c3. ADD PARTITION（未実測）。prep-ice はパーティション定義を持たない Iceberg テーブル
# なので、この構文自体が Iceberg にあるかどうかを測る。
if [ "$PREP_ICE_OK" = 0 ]; then
  run c3 "ALTER TABLE $DB.$TABLE_ICE ADD PARTITION (p = 'v')" keys
else
  skip c3 "$TABLE_ICE を作れなかったので未測定"
fi

# e2. SET LOCATION × Iceberg（issue の筆頭項目。#39 では Hive でしか測れなかった）。
# location を変えるので、この後 ice テーブルに触れる項目は無い（必ず最後に置く）。
if [ "$PREP_ICE_OK" = 0 ]; then
  run e2 "ALTER TABLE $DB.$TABLE_ICE SET LOCATION '${OUTPUT}43-loc-ice/'" keys
else
  skip e2 "$TABLE_ICE を作れなかったので未測定"
fi

# --- 本編: athena_local_probe_43_part（パーティション操作）--------------------

# c1. ADD PARTITION ... LOCATION（未実測）。
if [ "$PREP_PART_OK" = 0 ]; then
  run c1 "ALTER TABLE $DB.$TABLE_PART ADD PARTITION (p = 'v2') LOCATION '${OUTPUT}43-part-v2/'" keys
else
  skip c1 "$TABLE_PART を作れなかったので未測定"
fi

# c2. DROP PARTITION（未実測）。c1 が足したパーティションを落とすので、c1 の後に置く。
if [ "$PREP_PART_OK" = 0 ]; then
  run c2 "ALTER TABLE $DB.$TABLE_PART DROP PARTITION (p = 'v2')" keys
else
  skip c2 "$TABLE_PART を作れなかったので未測定"
fi

# --- 本編: RENAME 専用テーブル -------------------------------------------------

# b1. RENAME TO × Hive（未実測。SubstatementType が None のままでよいか確認する）。
if [ "$PREP_RN_HIVE_OK" = 0 ]; then
  run b1 "ALTER TABLE $DB.$TABLE_RN_HIVE RENAME TO $TABLE_RN_HIVE2" keys
else
  skip b1 "$TABLE_RN_HIVE を作れなかったので未測定"
fi

# b2. RENAME TO × Iceberg（未実測）。
if [ "$PREP_RN_ICE_OK" = 0 ]; then
  run b2 "ALTER TABLE $DB.$TABLE_RN_ICE RENAME TO $TABLE_RN_ICE2" keys
else
  skip b2 "$TABLE_RN_ICE を作れなかったので未測定"
fi

# --- table_format 列を後から埋める -------------------------------------------
# SHOW CREATE TABLE は作った直後にしか呼べない（後始末の DROP の後は失敗する）ので、
# 先に確かめておいた fmt を、同じテーブルに触れた行すべてへまとめて書き込む。

apply_table_format "prep-hive f1 d1 a1 e1" "$FMT_HIVE"
apply_table_format "prep-ice f2 d2 g1 a2 c3 e2" "$FMT_ICE"
apply_table_format "prep-part c1 c2" "$FMT_PART"
apply_table_format "prep-rn-hive b1" "$FMT_RN_HIVE"
apply_table_format "prep-rn-ice b2" "$FMT_RN_ICE"

# --- 後始末: 作った 5 テーブルを DROP TABLE ------------------------------------
# RENAME 済み（b1/b2 が SUCCEEDED）のものは新しい名前で DROP する。それが失敗した
# ときだけ、元の名前でも試す（RENAME が実は効いていなかった場合の保険）。
# summary.tsv には行を足さない（測る項目の設計表に無いため）。実行結果は
# cleanup-drops.txt に残す（バケット名・DB 名は含まないので summary.txt には
# ファイル名の案内だけ出す）。

: > "$RUN_DIR/cleanup-drops.txt"
CLEANUP_ALL_OK=1

if [ "$PREP_HIVE_OK" = 0 ]; then
  drop_table_final hive "$TABLE_HIVE" || CLEANUP_ALL_OK=0
fi
if [ "$PREP_ICE_OK" = 0 ]; then
  drop_table_final ice "$TABLE_ICE" || CLEANUP_ALL_OK=0
fi
if [ "$PREP_PART_OK" = 0 ]; then
  drop_table_final part "$TABLE_PART" || CLEANUP_ALL_OK=0
fi

if [ "$PREP_RN_HIVE_OK" = 0 ]; then
  if ! drop_table_final rn-hive "$TABLE_RN_HIVE2"; then
    echo "== cleanup-rn-hive: 新しい名前 ($TABLE_RN_HIVE2) での DROP に成功しませんでした。元の名前でも試します。"
    drop_table_final rn-hive-orig "$TABLE_RN_HIVE" || CLEANUP_ALL_OK=0
  fi
fi
if [ "$PREP_RN_ICE_OK" = 0 ]; then
  if ! drop_table_final rn-ice "$TABLE_RN_ICE2"; then
    echo "== cleanup-rn-ice: 新しい名前 ($TABLE_RN_ICE2) での DROP に成功しませんでした。元の名前でも試します。"
    drop_table_final rn-ice-orig "$TABLE_RN_ICE" || CLEANUP_ALL_OK=0
  fi
fi

# 後始末がすべて成功していれば（そもそも作られておらず対象が無い場合も含む）5 テーブル
# とももう存在しないので、trap での保険の DROP TABLE IF EXISTS は省く。1 本でも
# 成功しなかったものがあれば保険を残す（残しても DROP TABLE IF EXISTS をもう一度
# 投げるだけで、害は無い）。
...
# Hive のテーブル（_hive・_part・_rn_hive）が $OUTPUT の下に残したデータの key を、
# 消すためのヒントとして残す。バケット名・プレフィックスを含むので summary.txt には
# 入れない。
{
  echo "# Hive のテーブル（${PREFIX}_hive・${PREFIX}_part・${PREFIX}_rn_hive）のデータは"
  echo "# DROP TABLE では消えません。下の key を見て、要らなければ手で消してください"
  echo "# （例: aws s3 rm --recursive <パス>）。"
  for label in prep-hive prep-ice prep-part prep-rn-hive prep-rn-ice $LABELS; do
    echo "## $label"
...
write_summary_txt() {
  ...
  {
    echo "# issue #43: ALTER TABLE の未実測の亜種（SET LOCATION × Iceberg・ADD/DROP PARTITION・"
    echo "# REPLACE COLUMNS・RENAME TO・DROP COLUMNS 複数形）の実測"
    echo "# 実行日時: $(date -Iseconds)"
    echo "# StartQueryExecution を呼んだ回数（一時的な失敗の再試行込み。後始末の"
    echo "#   DROP TABLE も含む）: $(wc -l < "$START_CALL_FILE" | tr -d ' ')"
    echo "#   ※ GetQueryExecution / S3 への呼び出しはこの回数に含めない（課金には影響しない）。"
    echo "#   ※ trap で発動する後始末（ベストエフォートの DROP TABLE IF EXISTS）だけは"
    echo "#     この回数に含めない。正常終了で全テーブルの後始末が終わっていれば trap は何も呼ばない。"
    echo "# DDL: あり。作成: ${PREFIX}_hive・${PREFIX}_part・${PREFIX}_rn_hive（CTAS, Hive）/"
    echo "#       ${PREFIX}_ice・${PREFIX}_rn_ice（CTAS, Iceberg）の 5 テーブル。ALTER TABLE を"
    echo "#       hive/ice/part に最大 14 回、RENAME TO を rn_hive/rn_ice に 1 回ずつ、最後に"
    echo "#       5 テーブルを DROP（RENAME 済みは新しい名前、必要なら元の名前でも）。"
    echo "# 課金: 作るテーブルはどれも 0〜1 行、スキャンは無い（DDL と SELECT 1 だけ）。"
    echo "#       Athena の最小課金（10MB 相当）× クエリ数の見込み。"
...
PYEOF
    ...
    python3 - "$RUN_DIR" "prep-hive prep-ice prep-part prep-rn-hive prep-rn-ice $LABELS" "$OUTPUT_BUCKET" <<'PYEOF'
import json, os, sys
...
PYEOF
    echo
    echo "## 失敗した項目の理由（DB 名・出力先・バケット名は伏せてある）"
    for label in $ALL_LABELS; do
      if [ -s "$RUN_DIR/$label.reason.txt" ] && ! grep -q "^State: SUCCEEDED" "$RUN_DIR/$label.reason.txt"; then
        echo "### $label"
        # StateChangeReason には DB 名やテーブルの場所がそのまま載る。今回は FAILED を
        # 想定している項目（Hive の DROP COLUMNS、Iceberg の ADD PARTITION など）が
        # 複数あって必ずここに出るので、貼れる形にするため redact を通す
        # （measurement.md 要件 6）。伏せていない原文は <label>.reason.txt に残る。
        printf '%s\n\n' "$(redact "$(cat "$RUN_DIR/$label.reason.txt")")"
      fi
    done
  } > "$txt"
  echo "$txt"
...
echo "後始末の DROP TABLE の実行結果: $RUN_DIR/cleanup-drops.txt"
