#!/usr/bin/env python3
# issue #84 で作成（tools/ へ移す前の名前は 84-measure-raw-parse-errors.py）
...
raw-get-query-results.py を雛形にしていて、署名・応答の保存・要約の仕組みは
そのまま。違うのは次のところ。
...
def build_cases(output):
    ...
    plain = [
        ...
        # D. #87 で足した未実測の組み合わせ（2 ラウンド目。1〜46 と一緒に流す）
        (47, "MaxResults が false（真偽値 → Integer。true は TRUE_VALUE）", lwg, '{"MaxResults": false}'),
        (48, "NextToken が false（真偽値 → String）", lwg, '{"NextToken": false}'),
        (49, "NextToken が小数（小数 → String）", lwg, '{"NextToken": 1.5}'),
        (50, "ExecutionParameters が整数（整数 → 配列）", sqe,
         '{"QueryString": "SELECT", "ExecutionParameters": 1, "ClientRequestToken": "<TOKEN>", ' + rc + "}"),
        (51, "ExecutionParameters が真偽値（真偽値 → 配列）", sqe,
         '{"QueryString": "SELECT", "ExecutionParameters": true, "ClientRequestToken": "<TOKEN>", ' + rc + "}"),
        (52, "ExecutionParameters がオブジェクト（オブジェクト → 配列）", sqe,
         '{"QueryString": "SELECT", "ExecutionParameters": {}, "ClientRequestToken": "<TOKEN>", ' + rc + "}"),
        (53, "ResultConfiguration が整数（整数 → 構造体）", sqe,
         '{"QueryString": "SELECT", "ResultConfiguration": 1, "ClientRequestToken": "<TOKEN>"}'),
        (54, "ResultConfiguration が真偽値（真偽値 → 構造体）", sqe,
         '{"QueryString": "SELECT", "ResultConfiguration": true, "ClientRequestToken": "<TOKEN>"}'),
        (55, "ExecutionParameters の要素が null", sqe,
         '{"QueryString": "SELECT", "ExecutionParameters": [null], "ClientRequestToken": "<TOKEN>", ' + rc + "}"),
        (56, "ExecutionParameters の要素が配列", sqe,
         '{"QueryString": "SELECT", "ExecutionParameters": [["a"]], "ClientRequestToken": "<TOKEN>", ' + rc + "}"),
        (57, "入れ子の null（ResultConfiguration.OutputLocation が null。通れば OutputLocation 無しの扱い）", sqe,
         '{"QueryString": "SELECT", "ResultConfiguration": {"OutputLocation": null}, "ClientRequestToken": "<TOKEN>"}'),
        (58, "入れ子の型違い（ResultConfiguration.OutputLocation が数値）", sqe,
         '{"QueryString": "SELECT", "ResultConfiguration": {"OutputLocation": 1}, "ClientRequestToken": "<TOKEN>"}'),
        (59, "本文が数値", lwg, "1"),
        (60, "本文が真偽値", lwg, "true"),
        (61, "本文が 2 つの JSON（末尾にゴミ）", lwg, '{"MaxResults": 1} {}'),
    ]
    cases = []
...
def header_lines(stamp, region):
    return [
        ...
        "リージョン = {}".format(region),
        "本物への呼び出し 61 回、すべて 400 想定、クエリは流さない、DDL 無し、課金は増えない見込み",
