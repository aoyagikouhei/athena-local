#!/usr/bin/env bash
# issue #91 で作成（tools/ へ移す前の名前は 91-measure-iceberg-zero-insert.sh）
# 本物の Athena で、Iceberg のテーブルへの 0 行の INSERT が置く結果ファイルを実測する。
# issue #91 のための実測スクリプト。#35 の insert-location.sh を雛形にしている
# （run・loc_shape_of・poll_until_terminal などのヘルパはそのまま）。
#
# 測りたいこと（#35 は 0 行の INSERT を Hive のテーブルでしか測っていない）:
#   a  Iceberg テーブルへの 0 行の INSERT（争点）の OutputLocation・本体・.metadata・マニフェスト
#   b  Iceberg テーブルへの 1 行の INSERT（対照: 形式を固定して行数だけ変える）
#   c  Hive テーブルへの 0 行の INSERT（対照: 行数を固定して形式だけ変える。#35 の c の再現）
#   d  Hive テーブルへの 1 行の INSERT（対照: マニフェストが置かれる側の再現）
# あわせて、.metadata の更新件数（field 3）が 0 行のとき `18 00` として出るかを、
# summary の update_count 列（.metadata の先頭を protobuf として読んだ field 3 の値）で見る。
#
# **対照を同じラウンドに入れる**（#26 の教訓。別の日・別のラウンドとの差を条件による違いと
# 解釈しない）。テーブルが本当に Hive / Iceberg になったかは SHOW CREATE TABLE で裏取りする。
#
# 使い方:
#   tools/dev.sh OUTPUT=s3://your-bucket/prefix/ DB=your_db bash tools/measure/iceberg-zero-row-insert.sh
...
#   OUT_DIR      既定 ${DEV_HOST_HOME:-$HOME}/athena-iceberg-zero-insert-measurements（実名が入るのでリポジトリの外に出す）
...
#   <db>.athena_local_probe_91_hive（Hive）、_iceberg（Iceberg）の 2 テーブルを
#   CREATE TABLE AS SELECT で作り、それぞれに INSERT で 0 行と 1 行を書き込み、
#   最後に DROP TABLE IF EXISTS で消す（異常終了時も trap で消しにいく）。
#   同名のテーブルが既にあると壊すので、開始前に SHOW TABLES で存在を確かめ、
#   1 件でもあればテーブルを作らずに止まる。
#   Hive の CTAS（_hive）のデータは Athena の既定で $OUTPUT の下の tables/<id>/ に
#   置かれ、DROP TABLE では消えない（外部テーブルのため）。残った key の一覧を
#   cleanup-hints.txt に書き出すので、消したければそれを見て手で消すこと。
#   Iceberg のテーブル（_iceberg）は location を $OUTPUT の下の tables-probe-91-i/ に指定し、
#   is_external = false にしてあるので DROP TABLE でデータも消える。
#
# 課金について: 作るテーブルは 1 行、書き込むのも 1 行ずつで、実テーブルのスキャンは無い。
...
#
# summary の決め手になる列は loc_shape と update_count で、OutputLocation の末尾の形と
# .metadata の field 3 の値だけを出す。バケット名・プレフィックス・クエリ ID は出さず、
# `tables/<id>`／`<id>`／`<id>.csv`／`<id>.txt`／other のどれかに畳む。table_format 列は
# SHOW CREATE TABLE の本文に table_type='iceberg' が（大文字小文字を無視して）現れたかで
# iceberg / hive に畳む。keys 列は <id> を含む key の**件数**だけを出す（key そのものは貼らない）。
...
OUT_DIR=${OUT_DIR:-${DEV_HOST_HOME:-$HOME}/athena-iceberg-zero-insert-measurements}
POLL_TIMEOUT=${POLL_TIMEOUT:-180}
RETRY_MAX=${RETRY_MAX:-4}
RETRY_DELAY=${RETRY_DELAY:-5}
PROBE_DDL=${PROBE_DDL:-1}

PREFIX=athena_local_probe_91
# 作るテーブルの接尾辞（本編のラベル a〜d と紛れないよう別の名前にする）。
TABLES="hive iceberg"
# 本編で測る項目のラベル。
LABELS="a b c d"
...
printf 'label\tstate\tstatement_type\tloc_shape\tbody_bytes\tbody_content_type\tmetadata_bytes\tmetadata_content_type\tkeys\ttable_format\tnote\tupdate_count\n' > "$SUMMARY"
echo "出力先: $RUN_DIR"
...
run() {
  ...
  echo "== $label  state=$state  stype=$stype  loc_shape=$shape  body=$body_bytes($body_ct)  metadata=$meta_bytes($meta_ct)  keys=$keys  update_count=$update_count"
  printf '%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\n' \
    "$label" "$state" "$stype" "$shape" "$body_bytes" "$body_ct" \
    "$meta_bytes" "$meta_ct" "$keys" "-" "$(sanitize "$note")" "$update_count" >> "$SUMMARY"
  [ "$state" = SUCCEEDED ]
}
...
# Hive のテーブル（Athena の既定）。対照の c / d の書き込み先。
...
run prep-i "CREATE TABLE $DB.${PREFIX}_iceberg WITH (table_type = 'ICEBERG', location = '${OUTPUT}tables-probe-91-i/', is_external = false) AS SELECT 1 AS n, 'x' AS s" keys
PREP_I_OK=$?

# 実際にその形式になったかを裏取りする（この裏取りが無いと a / c の差を形式の違いと言えない）。
[ "$PREP_H_OK" = 0 ] && record_table_format hive prep-h
[ "$PREP_I_OK" = 0 ] && record_table_format iceberg prep-i

# --- 本編 --------------------------------------------------------------------

# a. 争点: Iceberg テーブルへの 0 行の INSERT。athena-local は `<id>`・本体無し・.metadata ありにしている。
if [ "$PREP_I_OK" = 0 ]; then
  run a "INSERT INTO $DB.${PREFIX}_iceberg SELECT * FROM (VALUES (3, 'z')) AS t(n, s) WHERE t.n < 0" keys
else
  skip a "Iceberg のテーブルを作れなかったので未測定"
fi

# b. 対照: Iceberg テーブルへの 1 行の INSERT（形式を固定して行数だけ変える。#35 の b の再現）。
if [ "$PREP_I_OK" = 0 ]; then
...
# c. 対照: Hive テーブルへの 0 行の INSERT（行数を固定して形式だけ変える。#35 の c の再現）。
if [ "$PREP_H_OK" = 0 ]; then
...
# d. 対照: Hive テーブルへの 1 行の INSERT（マニフェストが置かれる側の再現。#35 の a）。
if [ "$PREP_H_OK" = 0 ]; then
  run d "INSERT INTO $DB.${PREFIX}_hive VALUES (2, 'y')" keys
else
...
write_summary_txt() {
  ...
  {
    echo "# issue #91: Iceberg のテーブルへの 0 行の INSERT の結果ファイル（対照つき）"
    ...
    echo "# DDL: あり（${PREFIX}_hive / _iceberg の 2 テーブルを作成→DROP、それぞれに INSERT で 0 行と 1 行を書き込み）"
...
with open(sys.argv[1], newline="") as f:
    for row in csv.DictReader(f, delimiter="\t"):
        print(
            ...
            "keys={keys} table_format={table_format} update_count={update_count} note={note}".format(**row)
        )
PYEOF
    echo
