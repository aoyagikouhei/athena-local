#!/usr/bin/env bash
# issue #3 で作成（tools/ へ移す前の名前は 3-measure-client-request-token-extra.sh）
# 本物の Athena で、1回目の実測（client-request-token.sh）で測れなかった4項目
# だけを測る補助スクリプト（issue #3）。
#
# 1回目の実測でわかったこと（issue #3 のノートの「実測の結果」参照）:
...
#       （raw-client-request-token.py、本体スクリプトから切り出した独立ファイル）。
...
if [ ! -f "$RAW_SCRIPT" ]; then
  echo "見つかりません: $RAW_SCRIPT（raw-client-request-token.py と同じディレクトリに置いてください）" >&2
  exit 1
fi
...
# raw-client-request-token.py が書いた JSON（キーは snake_case）から query_execution_id を抜く。
extract_raw_id() {
  python3 -c 'import json, sys
try:
...
# 生 HTTP で StartQueryExecution を1回呼ぶ（raw-client-request-token.py に委譲）。
