#!/usr/bin/env bash
# issue #39 で作成（tools/ へ移す前の名前は 39-measure-alter-substatement.sh）
# 本物の Athena で、ALTER TABLE の SubstatementType 判定を実測する。issue #39 の
# ための実測スクリプト 3 ラウンド目。
#
# tools/measure/drop-table-format.sh を雛形にしている（run・
...
# 測る項目だけ差し替えた）。
#
# 出どころ（issue #39 のノートの「実測の結果」「計画レビューの結果」節）:
...
#   が ALTER_TABLE_ADD_COLUMN と誤判定される。計画レビュー（issue #39 のノートの指摘 A）は
...
# まとめて書き込む。drop-table-format.sh と同じやり方）。
#
# 使い方:
#   tools/dev.sh OUTPUT=s3://your-bucket/prefix/ DB=your_db bash tools/measure/alter-substatement.sh
...
#   OUT_DIR      既定 ${DEV_HOST_HOME:-$HOME}/athena-alter-substatement-measurements（実名が入るのでリポジトリの外）
...
#
# drop-table-format.sh から変えたところ（この issue のために足したもの）:
...
OUT_DIR=${OUT_DIR:-${DEV_HOST_HOME:-$HOME}/athena-alter-substatement-measurements}
