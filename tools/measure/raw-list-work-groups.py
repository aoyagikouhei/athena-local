#!/usr/bin/env python3
# issue #9 で作成（tools/ へ移す前の名前は 9-measure-raw-list-work-groups.py）
"""SigV4 を自前で署名した生 HTTP で、Athena の ListWorkGroups を何通りか呼ぶ。

issue #9（athena-local の ListWorkGroups 対応）の実測用。raw-client-request-token.py を
...
def call(region, credentials, payload):
    """1 ケース分を送って、結果を dict で返す（raw-client-request-token.py と同じ形）。"""
