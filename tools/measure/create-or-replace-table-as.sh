#!/usr/bin/env bash
# issue #93 で作成（tools/ へ移す前の名前は 93-measure-create-or-replace.sh）
# 本物の Athena が `CREATE OR REPLACE TABLE ... AS SELECT`（Trino だけの構文）を受け付けるかと、
# 受け付けるときの結果ファイル名・付随ファイル・SubstatementType を実測する。
# issue #93 のための実測スクリプト。#91 の iceberg-zero-row-insert.sh を雛形にしている。
#
# 測りたいこと（athena-local は `OR REPLACE` も CTAS とみなして `tables/<id>` にしている）:
#   a1  CREATE OR REPLACE TABLE <hive> AS SELECT      対象が無い状態（新規）
#   a2  同じ文をもう一度                              対象がある状態（置換）
#   b1  CREATE OR REPLACE TABLE <iceberg> AS SELECT   新規（WITH (table_type = 'ICEBERG', ...)）
#   b2  同じ文をもう一度                              置換
#   c   CREATE TABLE <ctas> AS SELECT                対照: 素の CTAS（`tables/<id>` のはず）
# 受け付けない（構文エラー）なら StartQueryExecution が InvalidRequestException を返し、
# summary に START_FAILED とその 1 行目（文言）が出る。そのときファイル名の問いは消える。
# 受け付けるなら OutputLocation の形・本体・.metadata・マニフェスト・SubstatementType を採り、
# 作られたテーブルの形式を SHOW CREATE TABLE で裏取りする。
#
# 使い方:
#   tools/dev.sh OUTPUT=s3://your-bucket/prefix/ DB=your_db bash tools/measure/create-or-replace-table-as.sh
...
#   OUT_DIR      既定 ${DEV_HOST_HOME:-$HOME}/athena-create-or-replace-measurements（実名が入るのでリポジトリの外に出す）
#   POLL_TIMEOUT 終端状態を待つ上限（秒）。既定 180
#   RETRY_MAX    名前解決・接続など一時的な失敗を再試行する回数の上限。既定 4
#   RETRY_DELAY  再試行の間隔（秒）。既定 5
#   PROBE_DDL    既定 1（測る）。0 にすると全項目を skip する（この実測は DDL そのものなので何も測れない）。
#
# ** このスクリプトが本物に対して行う破壊的な操作 **
#   <db>.athena_local_probe_93_orh（Hive）、_ori（Iceberg）、_ctas（対照）の 3 テーブルを
#   CREATE [OR REPLACE] TABLE AS SELECT で作り、最後に DROP TABLE IF EXISTS で消す
#   （異常終了時も trap で消しにいく）。
#   同名のテーブルが既にあると壊すので、開始前に SHOW TABLES で存在を確かめ、
#   1 件でもあればテーブルを作らずに止まる。
#   Hive のテーブル（_orh / _ctas）のデータは Athena の既定で $OUTPUT の下の tables/<id>/ に
#   置かれ、DROP TABLE では消えない（外部テーブルのため）。残った key の一覧を
#   cleanup-hints.txt に書き出すので、消したければそれを見て手で消すこと。
#   Iceberg のテーブル（_ori）は location を $OUTPUT の下の tables-probe-93-i/ に指定し、
#   is_external = false にしてあるので DROP TABLE でデータも消える。
#
# 課金について: 作るテーブルは 1 行で、実テーブルのスキャンは無い。
# Athena の最小課金（10MB 相当）× クエリ数の見込み。
...
#   <label>.start.err          StartQueryExecution の標準エラー（構文エラーで弾かれた証拠）
...
#
# summary の決め手になる列は state（START_FAILED なら受け付けない）と loc_shape、substatement_type。
# バケット名・プレフィックス・クエリ ID は出さず、`tables/<id>`／`<id>`／`<id>.csv`／`<id>.txt`／
# other のどれかに畳む。table_format 列は SHOW CREATE TABLE の本文に table_type='iceberg' が
# （大文字小文字を無視して）現れたかで iceberg / hive に畳む。keys 列は <id> を含む key の**件数**
# だけを出す。どれも実名を含まないので、summary.txt はそのまま貼れる（START_FAILED の note は
# 文言そのものなので、DB 名が入っていれば <DB> に置換される）。
...
OUT_DIR=${OUT_DIR:-${DEV_HOST_HOME:-$HOME}/athena-create-or-replace-measurements}
POLL_TIMEOUT=${POLL_TIMEOUT:-180}
RETRY_MAX=${RETRY_MAX:-4}
RETRY_DELAY=${RETRY_DELAY:-5}
PROBE_DDL=${PROBE_DDL:-1}

PREFIX=athena_local_probe_93
# 作るテーブルの接尾辞（本編のラベルと紛れないよう別の名前にする）。
TABLES="orh ori ctas"
# 本編で測る項目のラベル。
...
printf 'label\tstate\tstatement_type\tloc_shape\tbody_bytes\tbody_content_type\tmetadata_bytes\tmetadata_content_type\tkeys\ttable_format\tnote\tupdate_count\tsubstatement_type\n' > "$SUMMARY"
echo "出力先: $RUN_DIR"
...
run() {
  ...
  local id state stype loc sub shape note keys
  ...
  printf '%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\n' \
    ...
    "$meta_bytes" "$meta_ct" "$keys" "-" "$(sanitize "$note")" "$update_count" "$sub" >> "$SUMMARY"
  [ "$state" = SUCCEEDED ]
}
...
if [ "$PROBE_DDL" != 1 ]; then
  for label in $LABELS; do
    skip "$label" "PROBE_DDL=0 のため未測定（この実測は DDL そのものなので、何も測れない）"
  done
...
# a1. 争点: Hive の CREATE OR REPLACE TABLE ... AS（対象なし）。受け付けなければ START_FAILED になる。
run a1 "CREATE OR REPLACE TABLE $DB.${PREFIX}_orh AS SELECT 1 AS n, 'x' AS s" keys
A1_OK=$?

# a2. 対象がある状態でもう一度（置換）。a1 が通ったときだけ。
if [ "$A1_OK" = 0 ]; then
  run a2 "CREATE OR REPLACE TABLE $DB.${PREFIX}_orh AS SELECT 2 AS n, 'y' AS s" keys
else
  skip a2 "a1 が通らなかったので未測定（置換する対象が無い）"
fi

# b1. Iceberg の CREATE OR REPLACE TABLE ... AS（対象なし）。
run b1 "CREATE OR REPLACE TABLE $DB.${PREFIX}_ori WITH (table_type = 'ICEBERG', location = '${OUTPUT}tables-probe-93-i/', is_external = false) AS SELECT 1 AS n, 'x' AS s" keys
B1_OK=$?

# b2. 置換。b1 が通ったときだけ。
if [ "$B1_OK" = 0 ]; then
  run b2 "CREATE OR REPLACE TABLE $DB.${PREFIX}_ori WITH (table_type = 'ICEBERG', location = '${OUTPUT}tables-probe-93-i/', is_external = false) AS SELECT 2 AS n, 'y' AS s" keys
else
  skip b2 "b1 が通らなかったので未測定（置換する対象が無い）"
fi

# c. 対照: 素の CTAS。`tables/<id>` のはず（#26・#35 の再現）。
run c "CREATE TABLE $DB.${PREFIX}_ctas AS SELECT 1 AS n, 'x' AS s" keys

# 作られたテーブルが本当に Hive / Iceberg になったかを裏取りする。
[ "$A1_OK" = 0 ] && record_table_format orh a1
[ "$B1_OK" = 0 ] && record_table_format ori b1
record_table_format ctas c
...
{
  echo "# Hive のテーブル（${PREFIX}_orh / _ctas）のデータは DROP TABLE では消えません。"
...
write_summary_txt() {
  ...
  {
    echo "# issue #93: CREATE OR REPLACE TABLE ... AS を本物が受け付けるかと結果ファイル名（対照の CTAS つき）"
    ...
    echo "# DDL: あり（${PREFIX}_orh / _ori / _ctas の 3 テーブルを CREATE [OR REPLACE] TABLE AS で作成→DROP）"
    echo "# 課金: 作るテーブルは 1 行、実テーブルのスキャンは無い。"
    ...
    echo "#   table_format  SHOW CREATE TABLE で見た実際のテーブル形式（a1 / b1 / c の行に入れる）"
    echo "#   substatement_type  GetQueryExecution の SubstatementType（CTAS なら CREATE_TABLE_AS_SELECT のはず）"
...
with open(sys.argv[1], newline="") as f:
    for row in csv.DictReader(f, delimiter="\t"):
        print(
            ...
            "keys={keys} table_format={table_format} update_count={update_count} substatement_type={substatement_type} note={note}".format(**row)
        )
PYEOF
    echo
