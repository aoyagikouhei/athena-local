...
check_nginx_log_format() {
  ...
  if [ -z "$id" ] || [ "$state" != "SUCCEEDED" ]; then
    record "nginx ログの形式" SKIP "未測定: 既知のオブジェクトを作れなかった（id=${id:-無し} state=${state:-無し}）"
    return 1
  fi
...
# (6): 合否は SHOW 3 行の status だけ。準備の失敗（3.5.0 以下の既知の NoSuchKey）は INFO で別に数える。
judge_57() {
  local out="$1" id="$2" label line st pass=0 bad="" setup nsk
  ...
  JUDGE_DETAIL="SHOW 3 行のうち PASS $pass ${bad:+（$bad）}failures=$(failures_of "$out")"
  [ "$pass" = "3" ] && JUDGE_STATUS=PASS || JUDGE_STATUS=FAIL
}

# (3): 4 文とも SQLException（理由つき）、後続の SELECT 1 が通り、対照の .txt が 4 件あり、.txt* への GET が 0 件。
judge_111() {
  local out="$1" fetcher="$2" prefix="$3" nlog="$4" label line ok=0 bad="" sel=0 fl txt gets
  ...
  JUDGE_DETAIL="例外 $ok/4 ${bad:+（外れ: $bad）}SELECT1=$sel failures=${fl:-無し} .txt=$txt 件 GET(.txt*)=$gets 件"
...
judge_preflight_failed() {
  local out="$1" prefix="$2" nlog="$3" msg gets words
...
judge_run() {
  ...
  if grep -q '^PREFLIGHT failed: ' "$4"; then judge_preflight_failed "$4" "$7" "$6"; return; fi
