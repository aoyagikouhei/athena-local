...
#
# .claude/issue-notes/39-measure-alter-substatement.sh を雛形にしている（run・
...
# 書き込む。39-measure-drop-table-format.sh / 39-measure-alter-substatement.sh と同じやり方）。
# 5 テーブルすべてで確かめる（RENAME 専用の 2 本も、CTAS の WITH 指定が実際に効いたかどうかを
# 独立して裏取りするため）。
#
# 使い方:
#   OUTPUT=s3://your-bucket/prefix/ DB=your_db bash .claude/issue-notes/43-measure-alter-variants.sh
#
# 必要な環境変数:
...
#   OUT_DIR      既定 $HOME/athena-alter-variants-measurements（実名が入るのでリポジトリの外）
...
#
# 39-measure-alter-substatement.sh から変えたところ（この issue のために足したもの）:
...
OUT_DIR=${OUT_DIR:-$HOME/athena-alter-variants-measurements}
