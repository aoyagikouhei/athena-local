...
9-measure-raw-list-work-groups.py を雛形にしていて、署名・送信・応答の保存・要約の
...
使い方:
    OUTPUT=s3://your-bucket/prefix/ python3 83-measure-raw-get-query-results.py

botocore が要る。システムの python3 に入っていないときは、#9 と同じ順で実行方法を探す。

    uv run --with botocore python3 83-measure-raw-get-query-results.py
    ...
    OUTPUT=... /tmp/botocore-venv/bin/python3 83-measure-raw-get-query-results.py
...
def call(region, credentials, target, payload):
    """1 リクエスト分を送って、結果を dict で返す（9-measure-raw-list-work-groups.py と同じ形）。"""
