...
# 実測する。issue #92 のための実測スクリプト。#1 の 1-measure-txt.sh を雛形に、#35 の
...
#
# 使い方:
#   OUTPUT=s3://your-bucket/prefix/ DB=your_db bash .claude/issue-notes/92-measure-explain-variants.sh
#
# 必要な環境変数:
...
#   OUT_DIR      既定 $HOME/athena-explain-variants-measurements（実名が入るのでリポジトリの外に出す）
...
OUT_DIR=${OUT_DIR:-$HOME/athena-explain-variants-measurements}
