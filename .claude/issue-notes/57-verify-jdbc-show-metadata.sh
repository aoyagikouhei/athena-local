...
start_athena_local() {
  ...
  (
    ...
    exec env \
      ...
      TRINO_USER="athena-local-issue57" \
...
run_jdbc_case() {
  ...
  docker compose -f "$COMPOSE_FILE" run --rm \
    ...
    java -cp 'target/classes:target/dependency/*:$DRIVER_MOUNT' local.athenajdbccheck.Main '$fetcher' 57 2>&1
  " >"$out" 2>&1
  local rc=$?
  local failures
...
collect_metadata() {
  ...
  # 6 個 = \$partitions の SELECT × 3 ラウンド + ドライバが接続時に流す接続テストの SELECT × 3 ラウンド
  # （#5 で見つけた `-- Athena JDBC driver connection test\nSELECT 1`。#52 で先頭コメントを読み飛ばすようになり .csv になった）。
  record ".csv.metadata（対照 SELECT）" INFO "$csv_meta 個（\$partitions の SELECT × 3 + 接続テストの SELECT × 3 = 6 のはず）"
  return 0
}
