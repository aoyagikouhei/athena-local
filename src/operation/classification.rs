...
/// 大文字の語の並び（`athena_sql::words`。空白・コメントと、識別子の文字と記号の境目で切る）から、先頭の `(` を
/// 取り除いたもの。先頭のコメント（2026-09-18 実測）もキーワードの間のコメント（2026-09-22 実測。#52）も語にならない。
/// キーワードの直後に空白の無い `SELECT(1)`・`TABLE"t"` も空白ありの形と同じ語になる（2026-09-25 実測。#200）。
/// `(` だけの語は取り除くと空になるので落とし、`( SELECT` と `(SELECT` を同じ判定にする（#64 のレビューで見つかった。本物も `( SELECT 1 )` と改行を挟んだ形を
